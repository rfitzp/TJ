...
This paper is organized as follows. The adopted plasma equilibrium is described in Sect.~\ref{s2}.  In Sect.~\ref{s3}, the electron temperature perturbation due to an NTM is calculated in the outer region . The corresponding electron temperature perturbation in the inner region is calculated in Sect.~\ref{s4}. The
...
\end{align}
where  $r=a\,\hat{r}$. 
Here, the dimensionless functions $H_1(\hat{r})$, $H_2(\hat{r})$, and $H_2(\hat{r})$ control the Shafranov shifts, vertical elongations, and  triangularities of
...
Figures~\ref{figa} and \ref{fig2} show the magnetic flux-surfaces and profiles of an ITER-like  example plasma equilibrium characterized by 
$B_0=5.3$ T, $R_0=6.2$ m, $a=0.2$, $H_2(1)=1.0$, and $H_3(1)= 0.5$.
...
Note that the critical linear tearing stability index that must be exceeded before the stabilizing effect of average magnetic field-line curvature is overcome\,\cite{tear10} exceeds the linear tearing stability index for both surfaces. In other words, the $3$, $2$ and the $2$, $1$ are both stable to
classical tearing modes. On the other hand, the difference between the bootstrap parameter, $\alpha_b$, and the curvature parameter, $\alpha_c$, is positive
for both surfaces. In other words, the $3$, $2$ and the $2$, $1$ are both potentially unstable to
...
electron pressure, bootstrap parameter, and curvature parameter, respectively, at the $3$, $2$ and the $2$, $1$ rational surfaces. \label{t1} }
\end{table}


\begin{figure}
\centerline{\includegraphics[width=0.85\textwidth]{Flux.pdf}}
\caption{The blue and green curves show contours of constant $r$ and $\theta$, respectively, for an ITER-like example
equilibrium characterized by $B_0=5.3$ T, $R_0=6.2$ m, $a=0.2$, $H_2(1)=1.0$, and $H_3(1)= 0.5$. The red curves show
the locations of the $q=3/2$ and $q=2$ surfaces. \label{figa}}
\end{figure}

\begin{figure}
\centerline{\includegraphics[width=1.0\textwidth]{Profile.pdf}}
