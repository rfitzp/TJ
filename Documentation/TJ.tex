...
was first introduced in Ref.~\onlinecite{connor} in a calculation that allowed for three  poloidal harmonics coupled via toroidicity. The
inverse aspect-ratio expansion approach was extended in Ref.~\onlinecite{am1} in a calculation that allowed for seven poloidal harmonics coupled via toroidicity, flux-surface elongation, and
...
%,\\[0.5ex]
%0&= {\cal J}\,(J^{\,\phi}\,B^{\,r}-J^{\,r}\,B^{\,\phi}) = 0,\\[0.5ex]
%0&= {\cal J}\,(J^{\,r}\,B^{\,\phi}-J^{\,\theta}\,B^{\,r}) = 0,
\end{align}
...
\end{align}
But, from Eq.~(\ref{jac}), 
%it follows from Eq.~(\ref{jac}) that
%\begin{align}
%|\nabla r\cdot\nabla\theta\times\nabla\phi|^2 &= |\nabla r\times\nabla\theta\cdot\nabla\phi|^2= |\nabla r\times\nabla\theta|^2\,|\nabla\phi|^2\nonumber\\[0.5ex] &=\left[|\nabla r|^2\,|\nabla\theta|^2-(\nabla r \cdot\nabla\theta)^2\right]\frac{1}{R^{\,2}}= \frac{1}{r^2\,R^{\,4}},
%\end{align}
%which implies that
\begin{equation}
...
%\nonumber\\[0.5ex]
%&=n\,\pi^2\,{\rm i}\sum_{j=1,J}(\hat{Z}_j^{\,\ast}\,\psi_j-\psi_j^{\,\ast}\,\hat{Z}_j)=n\,\pi^2\,{\rm i}\,(\underline{\hat{Z}}^\dag\,\underline{\psi}-\underline{\psi}^\dag\,\underline{\hat{Z}}).
\end{align}
...
$\hat{r}$, $\theta$, $\phi$ coordinate system. This goal can be achieved using {\em orthogonal toroidal coordinates}, $\mu$, $\eta$, $\phi$, where\,\cite{morse}
...
\end{align}
Note that  $\hat{r}\sim{\cal O}(1)$ in the immediate vicinity of the plasma/vacuum boundary, which implies that $z\gg 1$. 
...
\nabla^2{\cal E}_m &= 0 &\mbox{between $\hat{r}=1$ and $z=1$},\label{e359}\\[0.5ex]
{\cal E}_m &= 0& \mbox{at $z=1$}.\label{e360}
\end{align}
Recall that
\begin{align}
\nabla^2 V^p &= 0 &\mbox{between $\hat{r}=1$ and $z=1$},\label{e361}\\[0.5ex]
...
\subsection{Summary}
In Sects.~\ref{svacsoln}--\ref{svacsoln1}, we found that if the algebraic quantities ${\cal P}_m^{\,m'}$, ${\cal Q}_m^{\,m'}$, ${\cal R}_m^{\,m'}$,
...
for $j=1,J$ and $k=1,K$. Our general solution is now free of arbitrary coefficients.
 Finally, we obtain the {\em homogeneous tearing mode dispersion relation},\cite{connor,cht,am1,pletz}
...
\end{equation}
It is clear that, for a given RMP amplitude, the RMP that maximizes the torque exerted at the $k$th rational surface has a
poloidal spectrum such that
\begin{equation}
b_{m_j}\propto \chi_{k\,j},
\end{equation}
and a magnetic scalar potential such that
\begin{align}
V^x_k(z,\eta)&\propto \chi_{k\,j_0}\,\frac{\sqrt{\pi}\,\Gamma(1/2-n)}{\sqrt{2}}\,(z-\cos\eta)^{1/2}\,Q_{-1/2}^{\,n}(z)\\[0.5ex]
...
\bibitem{bate} A.~Erd\'{e}lyi, W.~Magnus, F.~Oberhettinger, F.C.~Tricomi, {\em Higher Transcendental Functions}, vol.~1, sect.~1.7.3. 
