...
Previously, the TJ code either allowed for fixed boundary calculations (i.e., calculations in which the plasma displacement is constrained to be zero at the plasma boundary), or calculations in which the plasma is surrounded by an infinite vacuum region. Appendix~\ref{vacuum} describes a
...
$S^{\,2/3}\,{\mit\Delta}\omega\,\tau_H\gg |E_{kk'}|$, where $S$ is a typical Lundquist number at a rational surface, ${\mit\Delta}\omega$ the typical difference between the sum of the
...
The DCON ideal stability code\,\cite{dcon} implements three tests for ideal stability. The first test is to ensure that the plasma is stable to localized ideal interchange modes, and
involves checking that the ideal Mercier interchange parameter, $D_{I\,k}$ [see Eq.~(\ref{di})], is negative at every rational surface in the plasma. This test is usually easily passed
unless one of the rational surfaces  lies very close to the magnetic axis.
The second test is for fixed-boundary ideal modes (i.e., modes in which the
...
for any value of $r$ lying in the range $0\leq r\leq a$ then the plasma is unstable to a fixed-boundary ideal mode. The third test is for
...
of the total energy matrix are negative then the plasma is unstable to a free-boundary ideal mode.  The TJ code implements the first ideal stability test,  and, as described in Sect.~\ref{ideal}, 
implements the relatively straightforward third test, rather than the much more complicated second test. One question that we would like to address in
this paper is whether the third test is sufficient to detect ideal stability boundaries associated with $m=1/n=1$ internal-kink modes. 
...
The parameter $f_1$ is designed to take the value $+1$ if the 2/1 tearing mode co-rotates with the electron fluid at the 2/1 rational surface, $0$ if the mode
co-rotates with the E-cross-B fluid, and $-1$ if the mode co-rotates with the ion fluid, and to take appropriate linear intermediate values. 
...
\hline
\end{tabular}
\caption{Circular cross-section plasma with  $q_0=1.1$, $q_a=3.5$,  $\beta_0=0.1$, and $a=0.2$.  Variation of the elements of the free-boundary $n=1$ tearing stability matrix with 
