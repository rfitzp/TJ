...
%DIF DEL TJ1.tex    Sat May 10 19:39:06 2025
%DIF ADD TJ1a.tex   Thu Jun  5 13:11:15 2025
\usepackage{amsmath}           		          	
...
Previously, the TJ code either allowed for  \DIFdelbegin \DIFdel{fixed boundary calculations , or }\DIFdelend \DIFaddbegin \DIFadd{``fixed-boundary''  calculations---that is, calculations in which the plasma displacement is constrained to be zero at the plasma boundary---or ``free-boundary'' calculations---that is, }\DIFaddend calculations in which the plasma \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{displacement is unconstrained at the boundary,  and the plasma is }\DIFaddend surrounded by a vacuum region \DIFaddbegin \DIFadd{of infinite extent}\DIFaddend . Appendix~\ref{vacuum} describes a
new capability that allows for the presence of a perfectly conducting wall in the vacuum region. The radius of the wall, relative to the plasma, can be varied. \DIFaddbegin \DIFadd{Thus, the new
...
an }%DIFDELCMD < {\em %%%
\DIFdel{external-kink}%DIFDELCMD < }%%%
\DIFdel{\/ mode and an }%DIFDELCMD < {\em %%%
\DIFdel{internal-kink}%DIFDELCMD < }%%%
...
\DIFaddend The TJ calculations include all poloidal harmonics in the range $m=-10$ to $m=+20$. \DIFaddbegin \DIFadd{This choice was made because it corresponds to the range of poloidal mode numbers automatically chosen by STRIDE. As is
...
the tearing stability index of the  $m=2/n=1$ tearing mode) values calculated by the TJ code,\cite{tj} the TEAR code (which is a cylindrical tearing mode code \DIFaddbegin \DIFadd{that is part of the TJ package}\DIFaddend ), and
...
the $E_{11}$ values calculate by TEAR shows no such increase, because the ideal stability boundary is associated with \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend $m=3/n=1$ \DIFdelbegin \DIFdel{external-kink }\DIFdelend \DIFaddbegin \DIFadd{kink }\DIFaddend mode, and this mode does not couple to the $m=2/n=1$ tearing mode in a cylindrical plasma. 
...
\DIFaddbegin \DIFadd{Roughly speaking, the linear decoupling of the tearing mode dispersion relation requires
...
element of the $E_{kk'}$ matrix.\mbox{%DIFAUXCMD
\cite{book} }\hskip0pt%DIFAUXCMD
This criterion is easily satisfied in tokamak plasmas with realistic aspect-ratios }[\DIFadd{for which we expect $E_{kk'}\sim {\cal O}(1)$}]\DIFadd{, even 
in next-generation devices that will not rotate as strongly as present-day devices.\mbox{%DIFAUXCMD
\cite{book}
}\hskip0pt%DIFAUXCMD
}

\DIFaddend The marginal stability point of the $k$th tearing mode is $E_{kk}=  {\mit\Delta}_{k\,{\rm crit}}$, rather than $E_{kk}=0$,\cite{fkr,ara} because of the
...
The \DIFdelbegin \DIFdel{DCON ideal stability code \,\mbox{%DIFAUXCMD
\cite{dcon} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{TJ code }\DIFaddend implements two tests for ideal stability. The first test is \DIFdelbegin \DIFdel{for fixed-boundary ideal modes (i.e., modes in which the radial plasma displacement is constrained to be zero at the plasma boundary). This test involves calculating the perturbed ideal plasma potential energy
...
for any value of $r$ lying in the range $0\leq r\leq a$ then the plasmais unstable to a fixed-boundary ideal mode.The second test is for
...
because the magnetic perturbation extends into the vacuum region surrounding the plasma).
 This }\DIFdelend \DIFaddbegin \DIFadd{to ensure that the plasma is stable to localized ideal interchange modes, and
involves checking that the ideal Mercier interchange parameter, $D_{I\,k}$ }[\DIFadd{see Eq.~(\ref{di})}]\DIFadd{, is negative at every rational surface in the plasma.\mbox{%DIFAUXCMD
\cite{mercier,dcon} }\hskip0pt%DIFAUXCMD
This test is usually easily passed
unless one of the rational surfaces  lies very close to the magnetic axis.
 The second }\DIFaddend test  involves calculating the Hermitian perturbed ideal plasma potential energy
...
form the Hermitian  total energy matrix, $\underline{\underline{W}} =\underline{\underline{W}}_{\,p} +  \underline{\underline{W}}_{\,v}$.\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ideal} }\hskip0pt%DIFAUXCMD
}\DIFaddend If any of the real eigenvalues
of the total energy matrix are negative then the plasma is unstable to \DIFdelbegin \DIFdel{a free-boundary ideal mode}\DIFdelend \DIFaddbegin \DIFadd{an ideal mode.  Note that the second test is a necessary and sufficient test for ideal stability.\mbox{%DIFAUXCMD
\cite{freidberg}
}\hskip0pt%DIFAUXCMD
}

\subsection{\DIFadd{Internal and External Modes}}
\DIFadd{In cylindrical geometry, where there is no linear coupling between different poloidal harmonics, a rational surface acts rather like an ideal wall as far as an ideal instability is concerned.  This
...
\DIFadd{In toroidal geometry, where there is linear coupling between different poloidal harmonics, the situation is more complicated}\DIFaddend . \DIFdelbegin \DIFdel{As described in Sect. ~\ref{ideal}, the TJ code
implements the relatively straightforward second test, rather than the much more complicatedfirst test. One question that we would like to address in
this paper is whether the second test is sufficient to detect ideal stability boundaries associated with $m=1/n=1$ internal-kink modes.  }\DIFdelend \DIFaddbegin \DIFadd{It is still true that the resonant harmonic of
...
magnetic curvature effects. \DIFdelbegin \DIFdel{(Unfortunately, this is a little hard to see in the figure.) }\DIFdelend This is because $E_{11}$, despite being positive, falls below ${\mit\Delta}_{1\,{\rm crit}}\propto\beta_0$.  As $\beta_0$ further increases, 
...
The \DIFdelbegin \DIFdel{parameters }\DIFdelend \DIFaddbegin \DIFadd{parameter }\DIFaddend $f_1$ \DIFdelbegin \DIFdel{and $f_2$ are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend designed to take the \DIFdelbegin \DIFdel{values }\DIFdelend \DIFaddbegin \DIFadd{value }\DIFaddend $+1$ \DIFaddbegin \DIFadd{if the 2}\DIFaddend /\DIFdelbegin \DIFdel{0/$-1$ if the $2/1$ and
$3/1$ tearing modes, respectively,  co-rotate }\DIFdelend \DIFaddbegin \DIFadd{1 tearing mode co-rotates }\DIFaddend with the electron \DIFaddbegin \DIFadd{fluid at the 2}\DIFaddend /\DIFaddbegin \DIFadd{1 rational surface; the value $0$ if the mode
co-rotates with the }\DIFaddend E-cross-B \DIFdelbegin \DIFdel{/ion fluids at the surfaces at which they reconnect magnetic flux,  }\DIFdelend \DIFaddbegin \DIFadd{fluid; the value $-1$ if the mode co-rotates with the ion fluid; }\DIFaddend and to take appropriate \DIFdelbegin \DIFdel{linear }\DIFdelend intermediate values. 
\DIFaddbegin \DIFadd{The parameter $f_2$ is similar, except that it refers to the rotation of the 3/1 tearing mode at the 3/1 rational surface. 
}\DIFaddend It can be seen that, 
at intermediate values of $\beta_0$,  \DIFaddbegin \DIFadd{both }\DIFaddend $f_1$ and $f_2$ are \DIFdelbegin \DIFdel{both }\DIFdelend close to $+1$. This indicates that the modes in question are ``drift-tearing modes'' that co-rotate
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{According to Fig.~\ref{fig8}, the }\DIFaddend $2/1$ mode  has $\psi_{m=2}\neq 0$ at the $q=2$ surface, and $\psi_{m=3}=0$ at the $q=3$ surface.
...
an ideal stability boundary is approached, all of the decoupled tearing modes in the \DIFaddbegin \DIFadd{outer region of the }\DIFaddend plasma (with the same toroidal mode number as the ideal mode)
effectively morph into the ideal mode. \DIFaddbegin \DIFadd{This morphing occurs because the ratios of the coefficients of the small and large solutions at all rational surfaces in the plasma tend to
zero as the stability boundary is approached (essentially because the $F_{kk'}$ matrix becomes singular). (Of course, the ratios are all zero, by definition, for an ideal mode.) }\DIFaddend However, the growth-rate and real frequency of a given ``ideal-tearing-mode'' is determined by the 
...
real frequency. As is clear from Fig.~\ref{fig7}, in the present case, the \DIFdelbegin \DIFdel{ideal mode }\DIFdelend \DIFaddbegin \DIFadd{ideal-tearing-mode }\DIFaddend that reconnects flux closest to the edge of the plasma has the largest growth-rate. 
...
$\psi_{m=2}=0$.  \DIFaddbegin \DIFadd{Likewise, the $1/1$ mode has $\psi_{m=1}\neq 0$ at the $q=1$ rational surface, whereas the $2/1$ mode has 
$\psi_{m=1}=0$.
}\DIFaddend 
...
resistive layer solution at the rational surface at which it reconnects magnetic flux. As is clear from Fig.~\ref{fig16}, it is again the case that the \DIFdelbegin \DIFdel{ideal mode }\DIFdelend \DIFaddbegin \DIFadd{ideal-tearing-mode }\DIFaddend that reconnects flux closest to the edge of the plasma has the largest growth-rate. 
...
that the much more complicated first test for the ideal stability of fixed-boundary modes,\mbox{%DIFAUXCMD
\cite{dcon} }\hskip0pt%DIFAUXCMD
...
\DIFaddbegin \bibitem{book} \DIFadd{R.~Fitzpatrick, }{\em \DIFadd{Tearing Mode Dynamics in Tokamak Plasmas}} \DIFadd{(IOP, Bristol UK, 2023). 
}

\DIFaddend \bibitem{ggj} A.H.~Glasser, J.M.~Greene and J.L.~Johnson, Phys.\ Fluids {\bf 18}, 875 (1975).
...
\DIFaddFL{$m_{\rm min}$~~ }& \DIFaddFL{~~$m_{\rm max}$~~ }& \DIFaddFL{$E_{11}$ }& \DIFaddFL{$E_{12}$ }& \DIFaddFL{$E_{21}$ }& \DIFaddFL{$E_{22}$}\\ \hline
\DIFaddFL{-10  }&\DIFaddFL{20	      }&\DIFaddFL{11.57 }&\DIFaddFL{-10.53 }&\DIFaddFL{-10.53  }&\DIFaddFL{7.742 }\\
\DIFaddFL{-8   }&\DIFaddFL{18	      }&\DIFaddFL{11.57 }&\DIFaddFL{-10.53 }&\DIFaddFL{-10.53  }&\DIFaddFL{7.742 }\\
\DIFaddFL{-6   }&\DIFaddFL{16	      }&\DIFaddFL{11.57 }&\DIFaddFL{-10.53 }&\DIFaddFL{-10.53  }&\DIFaddFL{7.742 }\\
\DIFaddFL{-4   }&\DIFaddFL{14	      }&\DIFaddFL{11.57 }&\DIFaddFL{-10.53 }&\DIFaddFL{-10.53  }&\DIFaddFL{7.740 }\\
\DIFaddFL{-2   }&\DIFaddFL{12	      }&\DIFaddFL{11.56 }&\DIFaddFL{-10.51 }&\DIFaddFL{-10.51  }&\DIFaddFL{7.723 }\\
 \DIFaddFL{0   }&\DIFaddFL{10	      }&\DIFaddFL{11.80 }&\DIFaddFL{-10.81 }&\DIFaddFL{-10.81  }&\DIFaddFL{8.066 }\\
\hline
\end{tabular}
\caption{\DIFaddFL{Circular cross-section plasma with  $q_0=1.1$, $q_a=3.5$, $a=0.2$,  $\beta_0=0.1$, $p_p =2.0$,  and $a=0.2$.  Variation of the elements of the free-boundary $n=1$ tearing stability matrix with the 
