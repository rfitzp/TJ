...
! Program to input data from EFIT/EFIT.nc and write EFIT file EFIT.txt      
...
integer :: ncid, err, varid, file_unit, i, j
...
character(len=*), parameter :: filename = 'EFIT/EFIT.nc'
character(len=*), parameter :: string   = ' Output from TJ code: '
character(len=8)  :: date_str
character(len=10) :: time_str
character(len=48) :: comments_and_date
character(len=20) :: efit_name
...
comments_and_date = string // date_str
...
err = nf90_inq_varid (ncid, 'PSI', varid)
if (err /= nf90_noerr) then
   print *, 'Error getting variable ID for PSI'
   err =  nf90_close (ncid)
   stop
end if

err = nf90_get_var (ncid, varid, PSI)
if (err /= nf90_noerr) then
   print *, 'Error reading PSI data'
   err =  nf90_close (ncid)
   stop
end if
...
! Write EFIT file
! ---------------
efit_name = "EFIT/EFIT.txt"
  
open (unit = 10, file = efit_name, status = "replace", action = "write", iostat = file_unit)
    
! Check for errors in opening the file
if (file_unit /= 0) then
   print *, "Error opening EFIT/EFIT.txt"
   stop
end if

write (10, '(a48, 3i4)') comments_and_date, i3, NRBOX, NZBOX
