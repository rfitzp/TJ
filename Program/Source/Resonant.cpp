...
void TJ::JumpRational (int jres, double& r, Array<complex<double>,2> YY)
{
  ...
  if (fabs (nuL) > NULC)
      ...
      for (int k = 0; k < J; k++)
	{
      ...
      // ....................................
      // Calculate AL, AS, AC, BC, Pbar, Zbar
      // ....................................
      double                   Tminus, Tplus;
      ...
      for (int k = 0; k < J; k++)
	{
	  if (k != km)
	    {
	      akt[k] = - (Lmat(k, km) /L0  + Mmat(k, km)) /mm /sm;
	      bkt[k] = - (Nmat(k, km) /L0  + Pmat(k, km)) /mm /sm;
	    }
	  else
	    {
	      akt[k] = complex<double> (0., 0.);
      ...
      // ........................................
      // Calculate AL, AS, AC, AD, BD, Pbar, Zbar
      // ........................................
      ...
      for (int j = 0; j < K; j++)
	{
	  ...
	  for (int iter = 0; iter < ITERMAX; iter++)
	      ...
	      for (int k = 0; k < J; k++)
		{
		  if (k != km)
		    {
		      Pbar[k] = PPsi(k, j) - ak[k]*AL*logd;
		      Zbar[k] = ZZ  (k, j) - bk[k]*AL*logd;
		    }
		  else
		    {
		      Pbar[k] = complex<double> (0., 0.);
	      ...
	      for (int k = 0; k < J; k++)
		{
	      ...
	      AS = - (ZZ(km, j) - AL * nuL /L0 + DEL*logd*(BD + (gamL + delL*logd)*AL)) /bS /DEL;
