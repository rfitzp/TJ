...
Layer::Layer ()
{
  ...
  hmin = JSONData["Layer_control"]["hmin"].get<double> ();
  hmax = JSONData["Layer_control"]["hmax"].get<double> ();

  eps     = JSONData["Layer_control"]["eps"]    .get<double> ();
  smax    = JSONData["Layer_control"]["smax"]   .get<double> ();
  smin    = JSONData["Layer_control"]["smin"]   .get<double> ();
...
{
  printf ("Reading data from netcdf file Plots/TJ.nc:\n");
  ...
  try
    {
     ...
     {
       printf ("Error reading data from netcdf file Inputs/TJ.nc\n");
       printf ("%s\n", e.what ());
       exit (1);
     }
