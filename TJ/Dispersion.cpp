	...
	{     
	  if (mres[k] == 1)
	      ...
	      for (int i = 0; i < NDIAG; i++)
		{
	      ...
	      dTp [k] =   Tepres[k] * ISLAND * T0 + Tepres[k] * ISLAND * T0inf[k] - Tepres[k] * ISLAND;
	      dnp [k] =   nepres[k] * ISLAND * T0 + nepres[k] * ISLAND * T0inf[k] - nepres[k] * ISLAND;
	      ...
	      dTm [k] = - Tepres[k] * ISLAND * T0 + Tepres[k] * ISLAND * T0inf[k] + Tepres[k] * ISLAND; 
	      dnm [k] = - nepres[k] * ISLAND * T0 + nepres[k] * ISLAND * T0inf[k] + nepres[k] * ISLAND;
      ...
      for (int k = 0; k < nres; k++)
	printf ("m = %3d Psi = %10.3e Psip = (%10.3e, %10.3e) Psim = (%10.3e, %10.3e) dTp = %10.3e dTm = %10.3e delta = %10.3e\n",
		mres[k], Psik[k], real(PsTp[k]), imag(PsTp[k]), real(PsTm[k]), imag(PsTm[k]), dTp[k], dTm[k], delta[k]);
