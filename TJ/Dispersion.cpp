...
void TJ::FindDispersion ()
{
  ...
  if (TEMP)
    {
      // .....................................
      // Calculate island asymmetry parameters
      // .....................................
      for (int k = 0; k < nres; k++)
	{
	  double island;
      ...
      for (int k = 0; k < nres; k++)
	{
	  ...
	  if (mres[k] == 1)
	      ...
	      for (int i = 0; i < NDIAG; i++)
		{
	      ...
	      double T0p  = gsl_spline_eval (dThspline[k*Nh + 0],  1., dThacc[k*Nh + 0]);
	      double T1p  = gsl_spline_eval (dThspline[k*Nh + 1],  1., dThacc[k*Nh + 1]);
	      ...
	      PsTp[k] = - Tepres[k] * island * T1p * (rp * gp /qp) * mnp /Tpp /complex<double> (prp, pip);
	      Psnp[k] = - nepres[k] * island * T1p * (rp * gp /qp) * mnp /npp /complex<double> (prp, pip);
	      dTp [k] = Tepres[k] * island * T0p + Tepres[k] * island * T0inf[k] - Tepres[k] * island;
	      dnp [k] = nepres[k] * island * T0p + nepres[k] * island * T0inf[k] - nepres[k] * island;

	      double T0m  = gsl_spline_eval (dThspline[k*Nh + 0], -1., dThacc[k*Nh + 0]);
	      double T1m  = gsl_spline_eval (dThspline[k*Nh + 1], -1., dThacc[k*Nh + 1]);
	      ...
	      PsTm[k] = - Tepres[k] * island * T1m * (rm * gm /qm) * mnm /Tpm /complex<double> (prm, pim);
	      Psnm[k] = - nepres[k] * island * T1m * (rm * gm /qm) * mnm /npm /complex<double> (prm, pim);
	      dTm [k] = Tepres[k] * island * T0m + Tepres[k] * island * T0inf[k] + Tepres[k] * island;; 
	      dnm [k] = nepres[k] * island * T0m + nepres[k] * island * T0inf[k] + nepres[k] * island;
      ...
      for (int i = 0; i < NDIAG; i++)
	{
	  ...
	  for (int k = 0; k < nres; k++)
		...
		{
		  double T0 = gsl_spline_eval (dThspline[k*Nh + 0], x /island , dThacc[k*Nh + 0]);
		  
		  dneu(J, k, i) = nepres[k] * island * T0 + nepres[k] * island * T0inf[k] - nepres[k] * x;
		  dTeu(J, k, i) = Tepres[k] * island * T0 + Tepres[k] * island * T0inf[k] - Tepres[k] * x;
	  ...
	  for (int j = 0; j < J; j++)
		...
		if (mres[k] == 1 || MPOL[j] != mres[k])
		  ...
		  {
		    double T1  = gsl_spline_eval (dThspline[k*Nh + 1], x /island , dThacc[k*Nh + 1]);
