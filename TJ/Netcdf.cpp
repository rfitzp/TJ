...
// #############################################
void TJ::ReadIslandNetcdf (int k)
{
  char buffer[100];
  sprintf (buffer, "../Outputs/Island/Island%04d.nc", k);
  
  printf ("Reading data from netcdf file %s:\n", buffer);
   
  try
    {
      NcFile dataFile (buffer, NcFile::read);
...
void TJ::WriteNetcdf ()
{
  ...
  double* zzU_r   = new double[J*nres*NDIAG];
  double* zzU_i   = new double[J*nres*NDIAG];
  double* ccU_r   = new double[J*nres*NDIAG];
  double* ccU_i   = new double[J*nres*NDIAG];
  ...
  double* dTU_i   = new double[(J+1)*nres*NDIAG];
  double* nU_r    = new double[(J+1)*nres*NDIAG];
  double* nU_i    = new double[(J+1)*nres*NDIAG];
  double* TU_r    = new double[(J+1)*nres*NDIAG];
  double* TU_i    = new double[(J+1)*nres*NDIAG];
  ...
  double* zzV_r   = new double[nres*Nf*(Nw+1)];
  double* zzV_i   = new double[nres*Nf*(Nw+1)];
  double* ccV_r   = new double[nres*Nf*(Nw+1)];
  double* ccV_i   = new double[nres*Nf*(Nw+1)];
  double* bR_c    = new double[nres*Nf*(Nw+1)];
  double* bR_s    = new double[nres*Nf*(Nw+1)];
  double* bZ_c    = new double[nres*Nf*(Nw+1)];
  double* bZ_s    = new double[nres*Nf*(Nw+1)];
  double* bP_c    = new double[nres*Nf*(Nw+1)];
  ...
  double* dne_c   = new double[nres*Nf*(Nw+1)*NPHI];
  double* dTe_c   = new double[nres*Nf*(Nw+1)*NPHI];
  
  double* bReq_c   = new double[nres*2*Nf*NPHI];
  double* neeq_c   = new double[nres*2*Nf*NPHI];
  double* Teeq_c   = new double[nres*2*Nf*NPHI];
  double* dneeq_c  = new double[nres*2*Nf*NPHI];
  double* dTeeq_c  = new double[nres*2*Nf*NPHI];
  double* TeeqO_d  = new double[nres*2*Nf*NPHI];
  double* dTeeqO_d = new double[nres*2*Nf*NPHI];
  double* TeeqX_d  = new double[nres*2*Nf*NPHI];
  double* dTeeqX_d = new double[nres*2*Nf*NPHI];
  ...
  double* pXie_r   = new double[J*J*NDIAG];
  double* pXie_i   = new double[J*J*NDIAG];
  double* pxie_r   = new double[J*J*NDIAG];
  double* pxie_i   = new double[J*J*NDIAG];

  double* pPsiy_r  = new double[J*(Nw+1)];
  double* pPsiy_i  = new double[J*(Nw+1)];
  double* pJy_r    = new double[J*(Nw+1)];
  double* pJy_i    = new double[J*(Nw+1)];
  ...
  for (int j = 0; j < J; j++)
      ...
      for (int i = 0; i < NDIAG; i++)
	{
	  ...
	  zzU_r[cnt] = real (zu   (j, jp, i));
	  zzU_i[cnt] = imag (zu   (j, jp, i));
	  ccU_r[cnt] = real (chiu (j, jp, i));
	  ccU_i[cnt] = imag (chiu (j, jp, i));
	  xiU_r[cnt] = real (xiu  (j, jp, i));
	  xiU_i[cnt] = imag (xiu  (j, jp, i));
	  cnt++;
	}
	  ...
	  for (int l = 0; l <= Nw; l++)
	    {
	      ...
	      zzV_r[cnt] = real (zuv  (k, i, l));
	      zzV_i[cnt] = imag (zuv  (k, i, l));
	      ccV_r[cnt] = real (chiuv(k, i, l));
	      ccV_i[cnt] = imag (chiuv(k, i, l));
      ...
      if (TEMP)
	{
	  ...
	  for (int k = 0; k < nres; k++)
	      ...
	      for (int np = 0; np < NPHI; np++)
		{
		  ...
		  TeeqO_d [cnt] = TeeqdO (k, i, np);
		  dTeeqO_d[cnt] = dTeeqdO(k, i, np);
		  TeeqX_d [cnt] = TeeqdX (k, i, np);
		  dTeeqX_d[cnt] = dTeeqdX(k, i, np);
		  cnt++;
		}
	}
    }
	  ...
	  for (int i = 0; i < NDIAG; i++)
	    {
	      ...
	      pXie_r [cnt] = real (pXie (j, jp, i));
	      pXie_i [cnt] = imag (pXie (j, jp, i));
	      pxie_r [cnt] = real (pxie (j, jp, i));
	      pxie_i [cnt] = imag (pxie (j, jp, i));
	      cnt++;
	    }
	...
	for (int i = 0; i <= Nw; i++)
	  {
	    ...
	    pJy_r  [cnt] = real (pJy  (j, i));
	    pJy_i  [cnt] = imag (pJy  (j, i));
  ...
  try
    {
      ...
      vector<NcDim> full1_d;
      full1_d.push_back (z_d);
      full1_d.push_back (x_d);
      full1_d.push_back (d_d);
      ...
      NcVar kmp_x    = dataFile.addVar ("kmp",    ncDouble, kmp_d);
      kmp_x.putVar (kmp);
      NcVar km_x     = dataFile.addVar ("km",     ncDouble, kmp_d);
      km_x.putVar (km);
      ...
      zzui_x.putVar (ZZU_i);
      NcVar zxur_x = dataFile.addVar ("z_unrc_r",        ncDouble, full_d);
      zxur_x.putVar (zzU_r);
      NcVar zxui_x = dataFile.addVar ("z_unrc_i",        ncDouble, full_d);
      zxui_x.putVar (zzU_i);
      NcVar ccur_x = dataFile.addVar ("chi_unrc_r",      ncDouble, full_d);
      ccur_x.putVar (ccU_r);
      NcVar ccui_x = dataFile.addVar ("chi_unrc_i",      ncDouble, full_d);
      ccui_x.putVar (ccU_i);
      NcVar xir_x  = dataFile.addVar ("xi_unrc_r",       ncDouble, full_d);
      xir_x.putVar (xiU_r);
      ...
      if (TEMP)
	{
      ...
      Pn_x.putVar (Pres);
      NcVar Ls_x     = dataFile.addVar ("Ls_res",     ncDouble, x_d);
      Ls_x.putVar (Ls);
      NcVar LT_x     = dataFile.addVar ("LT_res",     ncDouble, x_d);
      LT_x.putVar (LT);
      NcVar Lc_x     = dataFile.addVar ("Lc_res",     ncDouble, x_d);
      Lc_x.putVar (Lc);
      NcVar bh_x     = dataFile.addVar ("betah_res",  ncDouble, x_d);
      bh_x.putVar (betah);
      NcVar ab_x     = dataFile.addVar ("alphab_res", ncDouble, x_d);
      ab_x.putVar (alphab);
      NcVar ac_x     = dataFile.addVar ("alphac_res", ncDouble, x_d);
      ac_x.putVar (alphac);
      NcVar pr_x     = dataFile.addVar ("Psi_res",    ncDouble, x_d);
      pr_x.putVar (Psik);
      NcVar dTp_x    = dataFile.addVar ("dTp_res",    ncDouble, x_d);
      dTp_x.putVar (dTp);
      NcVar dTm_x    = dataFile.addVar ("dTm_res",    ncDouble, x_d);
      dTm_x.putVar (dTm);
      NcVar del_x    = dataFile.addVar ("delta_res",  ncDouble, x_d);
      del_x.putVar (delta);
      NcVar wid_x    = dataFile.addVar ("width_res",  ncDouble, x_d);
      wid_x.putVar (width);
      ...
      if (VIZ)
	{
	  ...
	  zzvi_x.putVar (ZZV_i);
	  NcVar zxvr_x = dataFile.addVar ("z_unrc_eig_r",   ncDouble, v_d);
	  zxvr_x.putVar (zzV_r);
	  NcVar zxvi_x = dataFile.addVar ("z_unrc_eig_i",   ncDouble, v_d);
	  zxvi_x.putVar (zzV_i);
	  NcVar ccvr_x = dataFile.addVar ("chi_unrc_eig_r", ncDouble, v_d);
	  ccvr_x.putVar (ccV_r);
	  NcVar ccvi_x = dataFile.addVar ("chi_unrc_eig_i", ncDouble, v_d);
	  ccvi_x.putVar (ccV_i);
	  NcVar brc_x  = dataFile.addVar ("b_R_cos",        ncDouble, v_d);
	  brc_x.putVar (bR_c);
	  NcVar brs_x  = dataFile.addVar ("b_R_sin",        ncDouble, v_d);
	  brs_x.putVar (bR_s);
	  NcVar bzc_x  = dataFile.addVar ("b_Z_cos",        ncDouble, v_d);
	  bzc_x.putVar (bZ_c);
	  NcVar bzs_x  = dataFile.addVar ("b_Z_sin",        ncDouble, v_d);
	  bzs_x.putVar (bZ_s);
	  NcVar bpc_x  = dataFile.addVar ("b_phi_cos",      ncDouble, v_d);
	  bpc_x.putVar (bP_c);
	  NcVar bps_x  = dataFile.addVar ("b_phi_sin",      ncDouble, v_d);
	  bps_x.putVar (bP_s);
	  NcVar xic_x  = dataFile.addVar ("xi_cos",         ncDouble, v_d);
	  xic_x.putVar (xi_c);
	  ...
	  if (TEMP)
	    {
	      ...
	      NcVar TeeqdO_x  = dataFile.addVar ("Te_1^O",  ncDouble, vv_d);
	      TeeqdO_x.putVar (TeeqO_d);
	      NcVar dTeeqdO_x = dataFile.addVar ("dTe_1^O", ncDouble, vv_d);
	      dTeeqdO_x.putVar (dTeeqO_d);
	      NcVar TeeqdX_x  = dataFile.addVar ("Te_2^X",  ncDouble, vv_d);
	      TeeqdX_x.putVar (TeeqX_d);
	      NcVar dTeeqdX_x = dataFile.addVar ("dTe_2^X", ncDouble, vv_d);
	      dTeeqdX_x.putVar (dTeeqX_d);
	      ...
	      NcVar sigmaO_x = dataFile.addVar ("sigma_1^O", ncDouble, q_d);
	      sigmaO_x.putVar (sigmaO);
	      NcVar DeltaO_x = dataFile.addVar ("Delta_1^O", ncDouble, q_d);
	      DeltaO_x.putVar (DeltaO);
	      NcVar tauO_x   = dataFile.addVar ("tau_1^O",   ncDouble, q_d);
	      tauO_x.putVar (tauO);

	      NcVar sigmaX_x = dataFile.addVar ("sigma_2^X", ncDouble, q_d);
	      sigmaX_x.putVar (sigmaX);
	      NcVar DeltaX_x = dataFile.addVar ("Delta_2^X", ncDouble, q_d);
	      DeltaX_x.putVar (DeltaX);
	      NcVar tauX_x   = dataFile.addVar ("tau_2^X",   ncDouble, q_d);
	      tauX_x.putVar (tauX);
	    }
	}
      ...
      if (IDEAL)
	{
	  ...
	  NcVar ppsier_x = dataFile.addVar ("pPsi_e_r", ncDouble, ideal_d);
	  ppsier_x.putVar (pPsie_r);
	  NcVar ppsiei_x = dataFile.addVar ("pPsi_e_i", ncDouble, ideal_d);
	  ...
	  pzei_x.putVar (pZe_i);
	  NcVar pXer_x   = dataFile.addVar ("pXi_e_r",  ncDouble, ideal_d);
	  pXer_x.putVar (pXie_r);
	  NcVar pXei_x   = dataFile.addVar ("pXi_e_i",  ncDouble, ideal_d);
	  pXei_x.putVar (pXie_i);
	  NcVar pxer_x   = dataFile.addVar ("pxi_e_r",  ncDouble, ideal_d);
	  pxer_x.putVar (pxie_r);
	  NcVar pxei_x   = dataFile.addVar ("pxi_e_i",  ncDouble, ideal_d);
	  pxei_x.putVar (pxie_i);
	  ...
	  NcVar ppsiyr_x = dataFile.addVar ("pPsi_surface_r", ncDouble, surface_d);
	  ppsiyr_x.putVar (pPsiy_r);
	  NcVar ppsiyi_x = dataFile.addVar ("pPsi_surface_i", ncDouble, surface_d);
	  ppsiyi_x.putVar (pPsiy_i);
	  NcVar pjyr_x   = dataFile.addVar ("pJ_surface_r",   ncDouble, surface_d);
	  pjyr_x.putVar (pJy_r);
	  NcVar pjyi_x   = dataFile.addVar ("pJ_surface_i",   ncDouble, surface_d);
	  pjyi_x.putVar (pJy_i);
	  NcVar pxyr_x   = dataFile.addVar ("pXi_surface_r",  ncDouble, surface_d);
	  pxyr_x.putVar (pXiy_r);
	  NcVar pxyi_x   = dataFile.addVar ("pXi_surface_i",  ncDouble, surface_d);
      ...
      NcVar Te_r  = dataFile.addVar ("Te_res",     ncDouble, x_d);
      Te_r.putVar (Teres);
  ...
  delete[] Ltest;  delete[] MNtest; delete[] Ptest;  delete[] kmp;
  delete[] km;
  ...
  delete[] Teeq_c;  delete[] dTeeqO_d; delete[] TeeqO_d; delete[] dTeeqX_d;
  delete[] TeeqX_d;
     
  delete[] Psii_r; delete[] Psii_i; delete[] Zi_r;   delete[] Zi_i;
  delete[] Xii_r;  delete[] Xii_i;  delete[] Chii_r; delete[] Chii_i;
  delete[] xii_r;  delete[] xii_i;  delete[] xie_r;  delete[] xie_i;

  delete[] Uvec_r; delete[] Uvec_i; delete[] Umat_r; delete[] Umat_i;
  delete[] Uant_r; delete[] Uant_i; delete[] Psie_r; delete[] Psie_i;
  delete[] Ze_r;   delete[] Ze_i;   delete[] Xie_r;  delete[] Xie_i;
  delete[] Psiy_r; delete[] Psiy_i; delete[] Jy_r;   delete[] Jy_i;
  delete[] Xiy_r;  delete[] Xiy_i;

  delete[] Psis_r; delete[] Psis_i; delete[] Psiz_r; delete[] Psiz_i;

  delete[] pUvec_r; delete[] pUvec_i; delete[] pUmat_r; delete[] pUmat_i;
  delete[] pUant_r; delete[] pUant_i; delete[] pPsie_r; delete[] pPsie_i;
  delete[] pZe_r;   delete[] pZe_i;   delete[] pXie_r;  delete[] pXie_i;
  delete[] pPsiy_r; delete[] pPsiy_i; delete[] pJy_r;   delete[] pJy_i;
