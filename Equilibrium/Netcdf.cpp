...
void Equilibrium::WriteNetcdf (double sa)
{
  ...
  try
      ...
      if (VIZ)
	{
	  ...
	  NcVar Weeq_x  = dataFile.addVar ("We_eq",    ncDouble, e_d);
	  Weeq_x.putVar (Weeq);
	  NcVar wUeq_x  = dataFile.addVar ("wU_eq",    ncDouble, e_d);
	  wUeq_x.putVar (wUeq);
	  NcVar wLeq_x  = dataFile.addVar ("wL_eq",    ncDouble, e_d);
	  wLeq_x.putVar (wLeq);
	  NcVar wUHeq_x = dataFile.addVar ("wUH_eq",   ncDouble, e_d);
	  wUHeq_x.putVar (wUHeq);
	}
