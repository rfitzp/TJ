...
void Vertical::WriteNetcdf ()
{
  ...
  double* Umat_r = new double[K*K];
  double* Umat_i = new double[K*K];
  double* Uant_r = new double[K*K];
  double* Uant_i = new double[K*K];
  double* Uvec_r = new double[K*K];
  double* Uvec_i = new double[K*K];
  double* Psie_r = new double[J*K*NDIAG];
  double* Psie_i = new double[J*K*NDIAG];
  ...
  double* pUant_r = new double[K*K];
  double* pUant_i = new double[K*K];
  double* pUvec_r = new double[K*K];
  double* pUvec_i = new double[K*K];
  double* pPsie_r = new double[J*K*NDIAG];
  double* pPsie_i = new double[J*K*NDIAG];
  ...
  for (int j = 0; j < J; j++)
	...
	{
	  PPPsi_r[cnt] = real (Psii(j, jp, i));
	  PPPsi_i[cnt] = imag (Psii(j, jp, i));
	  ZZZ_r  [cnt] = real (Zi  (j, jp, i));
	  ZZZ_i  [cnt] = imag (Zi  (j, jp, i));
	  cnt++;
	}
  ...
  try
    {
      ...
      vector<NcDim> reduce_d;
      reduce_d.push_back (k_d);
      reduce_d.push_back (k_d);
      ...
      mpol_x.putVar (MPOL);
      NcVar mpox_x = dataFile.addVar ("mpox", ncInt, j_d);
      mpox_x.putVar (MPOX);
      ...
      NcVar wvacr_x  = dataFile.addVar ("Umat_r",  ncDouble, reduce_d);
      wvacr_x.putVar (Umat_r);
      NcVar wvaci_x  = dataFile.addVar ("Umat_i",  ncDouble, reduce_d);
      wvaci_x.putVar (Umat_i);
      NcVar wantr_x  = dataFile.addVar ("Uant_r",  ncDouble, reduce_d);
      wantr_x.putVar (Uant_r);
      NcVar wanti_x  = dataFile.addVar ("Uant_i",  ncDouble, reduce_d);
      wanti_x.putVar (Uant_i);
      NcVar wval_x   = dataFile.addVar ("Wval",    ncDouble, k_d);
      wval_x.putVar  (Wval);
      NcVar vval_x   = dataFile.addVar ("Vval",    ncDouble, k_d);
      vval_x.putVar  (Vval);
      NcVar uval_x   = dataFile.addVar ("Uval",    ncDouble, k_d);
      uval_x.putVar  (Uval);
      NcVar wvecr_x  = dataFile.addVar ("Uvec_r",  ncDouble, reduce_d);
      wvecr_x.putVar (Uvec_r);

      NcVar pwvacr_x  = dataFile.addVar ("pUmat_r",  ncDouble, reduce_d);
      pwvacr_x.putVar (pUmat_r);
      NcVar pwvaci_x  = dataFile.addVar ("pUmat_i",  ncDouble, reduce_d);
      pwvaci_x.putVar (pUmat_i);
      NcVar pwantr_x  = dataFile.addVar ("pUant_r",  ncDouble, reduce_d);
      pwantr_x.putVar (pUant_r);
      NcVar pwanti_x  = dataFile.addVar ("pUant_i",  ncDouble, reduce_d);
      ...
      NcVar pwvecr_x  = dataFile.addVar ("pUvec_r",  ncDouble, reduce_d);
      ...
      NcVar dW_x  = dataFile.addVar ("delta_W",   ncDouble, k_d);
      dW_x.putVar (deltaW);
      NcVar dWv_x = dataFile.addVar ("delta_W_p", ncDouble, k_d);
      dWv_x.putVar (deltaWp);
      NcVar dWp_x = dataFile.addVar ("delta_W_v", ncDouble, k_d);
      dWp_x.putVar (deltaWv);

      NcVar pdW_x  = dataFile.addVar ("pdelta_W",   ncDouble, k_d);
      pdW_x.putVar (pdeltaW);
      NcVar pdWv_x = dataFile.addVar ("pdelta_W_p", ncDouble, k_d);
      pdWv_x.putVar (pdeltaWp);
      NcVar pdWp_x = dataFile.addVar ("pdelta_W_v", ncDouble, k_d);
