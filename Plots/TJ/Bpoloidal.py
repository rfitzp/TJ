...
# Plots B_poloidal and B_toroidal in R, Z plane.
...
ds1   = nc.Dataset(fn1)
rres  = ds1['r_res']
bpol  = np.asarray(ds1['B_poloidal'])
btor  = np.asarray(ds1['B_toroidal'])

fig = plt.figure (figsize = (12.5, 6.0))
fig.canvas.manager.set_window_title (r'TJ Code: B_poloidal (R, Z) and B_toroidal (R, Z)')
...
cp = plt.contourf (RR, ZZ, bpol, ncont, cmap = ReBu)

plt.contour (RR, ZZ, bpol, rres, levels = 20, colors = 'white', linewidths = 0.5)  
...
plt.contour (RR, ZZ, btor, rres, levels = 20, colors = 'white', linewidths = 0.5)  
