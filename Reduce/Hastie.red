...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
x := - r*cos(w) - eps*d1 + eps*e1*cos(w) + eps*t1*cos(2*w) + eps**2*p2*cos(w)$
z :=   r*sin(w)          + eps*e1*sin(w) + eps*t1*sin(2*w) - eps**2*p2*sin(w)$
...
q2   := r**3/8 + r*d1/2 - (2-1)*e1**2/(2*r) - (3-1)*t1**2/(2*r);
p2   := q2$
...
gs1  := coeffn(fcos(gs,t,1),eps,1)$
gs2  := coeffn(fcos(gs,t,2),eps,1)$
gs3  := coeffn(fcos(gs,t,3),eps,1)$
...
c0   := coeffn(gs1,df(d1,r,2),0)$
c1   := coeffn(gs1,df(d1,r,2),1)$
d1pp := -c0/c1;

write "l=2 shaping equation:";
c0   := coeffn(gs2,df(e1,r,2),0)$
c1   := coeffn(gs2,df(e1,r,2),1)$
e1pp := -c0/c1;

write "l=3 shaping equation:";
c0   := coeffn(gs3,df(t1,r,2),0)$
c1   := coeffn(gs3,df(t1,r,2),1)$
t1pp := -c0/c1;
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Only need to retain harmonic terms up to l=3
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
let cos(4*t) = 0$
let cos(5*t) = 0$
let cos(6*t) = 0$
...
c1 := coeffn(ex,eps,1)$
c2 := coeffn(ex,eps,2)$
c2 := sub(cos(t)=0,cos(2*t)=0,cos(3*t)=0,sin(t)=0,sin(2*t)=0,sin(3*t)=0,c2)$
...
order d1,df(d1,r),df(d1,r,2),e1,df(e1,r),df(e1,r,2),t1,df(t1,r),df(t1,r,2)$
...
lam00 := - cm00/bm00$
lam02 := - (cm02 - cm00*bm02/bm00)/bm00$
...
begin scalar c0$
c0 := ex$
c0 := sub(cos(t)=0,cos(2*t)=0,cos(3*t)=0,sin(t)=0,sin(2*t)=0,sin(3*t)=0,c0)$
return c0$
end$
...
d12  :=   (3/2)*df(d1,r)**2
        + (3/2)*df(e1,r)**2 - ((2*2-1)/2)*(e1/r)**2
	+ (3/2)*df(t1,r)**2 - ((3*3-1)/2)*(t1/r)**2$

lm00_target := m*m$
lm02_target := m*m*(- 3*r*r/4 - d1 + d12) + n*n*r*r$
...
pm0  := av(em0 + lam0*(dm0 - r*n*df(q,r)) - (m-n*q)*r*df(lam0,r))/n$
...
res1 := (r**2/q**3)*(2-sh) + (sh/q)*(3*r**2/4 + d1 - d12) - (2/q)*((3/2)*r**2 + d1 + df(d1,r)*r - (2/3)*d12)$
res2 :=   2*(2*2-1) * (df(e1,r)**2 - (11/3)*df(e1,r)*e1/r + 2*2*(e1/r)**2 - (1-sh)*(df(e1,r)*e1/r + (1/3)*(e1/r)**2))
        + 2*(3*3-1) * (df(t1,r)**2 - (11/3)*df(t1,r)*t1/r + 3*3*(t1/r)**2 - (1-sh)*(df(t1,r)*t1/r + (1/3)*(t1/r)**2))$
res3 := (n/m)*r*df(r*df(r2q,r),r) - df(r2q,r)**2 - r*df(r*pp,r) + m**2*(7*r**2/4 + d1 + 3*r*df(d1,r) + d12)$

pm02_target :=   (2*pp*r)*(1-q**2)
	       - (nmq0/m)*(2*pp*r)*(2-sh)
	       - (nmq0/m)*(r*q)*df(res1,r)
	       + (nmq0/m)*res2
	       + (nmq0/m)**2*res3$
...
lmp11_target :=   m*(m+1)*df(d1,r)$
mmp11_target := - m*nmq0*pp*q**2 + m*nmqp1*(r - df(d1,r)*(1-sh))$
nmp11_target := - (m+1)*nmqp1*pp*q**2 + (m+1)*nmq0*(r - df(d1,r)*(1-sh))$
pmp11_target := - (1+sh)*pp*q**2 + nmq0*nmqp1*(r + df(d1,r))$
...
lmm11_target :=   m*(m-1)*df(d1,r)$
mmm11_target :=   m*nmq0*pp*q**2 - m*nmqm1*(r - df(d1,r)*(1-sh))$
nmm11_target :=   (m-1)*nmqm1*pp*q**2 - (m-1)*nmq0*(r - df(d1,r)*(1-sh))$
pmm11_target := - (1+sh)*pp*q**2 + nmq0*nmqm1*(r + df(d1,r))$
...
lmp21_target := - m*(m+2)*df(e1,r)$
mmp21_target :=   (1/2)*m    *nmqp2*((1-sh)*df(e1,r) - (2*2-1)*e1/r)$
nmp21_target :=   (1/2)*(m+2)*nmq0 *((1-sh)*df(e1,r) - (2*2-1)*e1/r)$
pmp21_target := - nmqp2*nmq0*df(e1,r)$
...
lmm21_target := - m*(m-2)*df(e1,r)$
mmm21_target := - (1/2)*m    *nmqm2*((1-sh)*df(e1,r) - (2*2-1)*e1/r)$
nmm21_target := - (1/2)*(m-2)*nmq0 *((1-sh)*df(e1,r) - (2*2-1)*e1/r)$
pmm21_target := - nmqm2*nmq0*df(e1,r)$
...
lmp31_target := - m*(m+3)*df(t1,r)$
mmp31_target :=   (1/3)*m    *nmqp3*((1-sh)*df(t1,r) - (3*3-1)*t1/r)$
nmp31_target :=   (1/3)*(m+3)*nmq0 *((1-sh)*df(t1,r) - (3*3-1)*t1/r)$
pmp31_target := - nmqp3*nmq0*df(t1,r)$
...
lmm31_target := - m*(m-3)*df(t1,r)$
mmm31_target := - (1/3)*m    *nmqm3*((1-sh)*df(t1,r) - (3*3-1)*t1/r)$
nmm31_target := - (1/3)*(m-3)*nmq0 *((1-sh)*df(t1,r) - (3*3-1)*t1/r)$
pmm31_target := - nmqm3*nmq0*df(t1,r)$
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Calculate special case coupling coefficients
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
mmp10_target := sub(m=+1,mmm11_target) + (2-sh)*df(d1,r)$
mmm10_target := sub(m=-1,mmp11_target) - (2-sh)*df(d1,r)$
nm0p1_target := sub(m= 0,nmp11_target) - (2-sh)*df(d1,r)$
nm0m1_target := sub(m= 0,nmm11_target) + (2-sh)*df(d1,r)$
pm0p1_target := sub(m= 0,pmp11_target) - (2-sh)*(   n*q*pp*q**2 + (1-n*q)*(r - (1-sh)*df(d1,r)))$
pm0m1_target := sub(m= 0,pmm11_target) - (2-sh)*( - n*q*pp*q**2 + (1+n*q)*(r - (1-sh)*df(d1,r)))$
...
mmp20_target := sub(m=+2,mmm21_target) - 2*(2-sh)*df(e1,r)$
mmm20_target := sub(m=-2,mmp21_target) + 2*(2-sh)*df(e1,r)$
nm0p2_target := sub(m= 0,nmp21_target) + 2*(2-sh)*df(e1,r)$
nm0m2_target := sub(m= 0,nmm21_target) - 2*(2-sh)*df(e1,r)$
pm0p2_target := sub(m= 0,pmp21_target) - (1/2)*(2-sh)*(2-n*q)*((1-sh)*df(e1,r) - (2*2-1)*e1/r)$
pm0m2_target := sub(m= 0,pmm21_target) - (1/2)*(2-sh)*(2+n*q)*((1-sh)*df(e1,r) - (2*2-1)*e1/r)$
...
mmp30_target := sub(m=+3,mmm31_target) - 3*(2-sh)*df(t1,r)$
mmm30_target := sub(m=-3,mmp31_target) + 3*(2-sh)*df(t1,r)$
nm0p3_target := sub(m= 0,nmp31_target) + 3*(2-sh)*df(t1,r)$
nm0m3_target := sub(m= 0,nmm31_target) - 3*(2-sh)*df(t1,r)$
pm0p3_target := sub(m= 0,pmp31_target) - (1/3)*(2-sh)*(3 - n*q)*((1-sh)*df(t1,r) - (3*3-1)*t1/r)$
pm0m3_target := sub(m= 0,pmm31_target) - (1/3)*(2-sh)*(3 + n*q)*((1-sh)*df(t1,r) - (3*3-1)*t1/r)$
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Determine equilibrium functions
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
g4p_target  := - (3*r*r/2 + 2*r*df(d1,r) + df(d1,r)**2
                  + df(e1,r)**2 + 2*(2*2-1)*df(e1,r)*e1/r - (2*2-1)*(e1/r)**2
		  + df(t1,r)**2 + 2*(3*3-1)*df(t1,r)*t1/r - (3*3-1)*(t1/r)**2)*r/q**2
               + (- g2 - 3*r**2/4 + r**2/q**2 - d1 + d12)*(2-sh)*r/q**2
	       + pp * (g2 + r**2/2 + r**2/q**2 + 2*d1 + 3*r*df(d1,r))$
d1pp_target := - (3-2*sh)*df(d1,r)/r + 1 - 2*pp*q**2/r$
e1pp_target := - (3-2*sh)*df(e1,r)/r + (2*2-1)*e1/r**2$
t1pp_target := - (3-2*sh)*df(t1,r)/r + (3*3-1)*t1/r**2$

write "Equilibrium residuals:";
dg2p  := g2p  - g2p_target;
dg4p  := g4p  - g4p_target;
dd1pp := d1pp - d1pp_target;
de1pp := e1pp - e1pp_target;
dt1pp := t1pp - t1pp_target;
