...
This paper is organized as follows. The adopted plasma equilibrium is described in Sect.~\ref{s2}.  In Sect.~\ref{s3}, the calculation of the perturbed electron temperature associated with  an NTM  in the outer region is described. The corresponding calculation of the perturbed electron temperature  in the inner region is described  in Sect.~\ref{s4}. The
...
calculated in Sect.~\ref{s5}. 
The calculation of the ECE signal due to the NTM is described in Sect.~\ref{s6}.
...
The TJ code employs the conventional cylindrical coordinates $R$, $\phi$, $Z$, as well as a flux-coordinate system, $r$, $\theta$, $\phi$, where $r$ is a 
magnetic flux-surface label which can be interpreted as the
...
$B_0=5.3$ T, $R_0=6.2$ m, $a=0.2$, $H_2(1)=1.0$,  $H_3(1)= 0.5$, $q(0)=1.01$,  $q(1)=3.6$,
and $p_2(0)=0.2$. Here, $\hat{r}=r/a$. The normalized plasma inductance is
...
resonant at multiple rational surfaces within the plasma, but is assumed to only reconnect magnetic flux at the $l$th surface, of normalized minor
radius $r_l$, at which $q(r_l)=m_l/n$. 

The perturbed magnetic field in the outer region consists of a single toroidal harmonic, but many linearly-coupled poloidal
harmonics, and is specified by a set of normalized radial eigenfunctions, $\hat{\psi}_{m_j}(r)$, where $j$ indexes the poloidal harmonics included
...
asymmetry parameter, $\delta$. An island chain with $\delta>0$ (which is what we generally expect) is such that the magnetic X-points are shifted
radially outward (in $r$) from the rational surface by $\delta\,W/\sqrt{8}$, whereas the O-points are shifted radially inward by $\delta\,W/\sqrt{8}$. (See Fig.~\ref{fig1}.) The island asymmetry is produced
...
O-point is less than about twice the radial standard deviation of the current drive profile (assuming that the island width is itself less than or of order twice the
standard deviation). (See Sect.~\ref{rotate}.)
...
is the equilibrium electron temperature profile, $T_{e\,0}'(r) = dT_{e\,0}/dr$, $g(r)/R$ is the normalized equilibrium toroidal magnetic field-strength, and 
...
\end{equation}
where $B$ is the (unnormalized) local magnetic field-strength, $e$ the magnitude of the electron charge, and $m_e$ the electron rest mass.
...
\exp\left[-\frac {(R-R_\omega+{\mit\Delta})^2}{2\,\sigma^2}\right]&R\leq R_\omega\nonumber\\[0.5ex]
&= 0& R>R_\omega.
\end{align}
...
NTM temperature perturbation is actually rotating toroidally (because the NTM island chain is rotating), different toroidal angles translate to different measurement times. (See the discussion on island rotation in Sect.~\ref{rotate}.) It can be seen that the temperature profile measured by the relativistically downshifted and 
...
the edge of the plasma). It is clear from the figure that the algorithm is quite capable of detecting island chains of widths as small as 1\% of the plasma minor radius. (This detection threshold neglects the effect of experimental noise, and should be considered as an absolute, rather than a practical, threshold.
Note, also, that the critical island width needed for detection by ECE is greater than that needed to flatten the electron temperature profile. See Table~\ref{t1}.) 
...
as small as $1\%$ of the plasma minor radius (in the absence of noise). (See Sect.~\ref{s6}.) Moreover, this measurement is not
...
The flattening of the electron temperature profile inside the separatrix of an NTM magnetic island chain can also be detected by means of
a Thomson scattering diagnostic \cite{thom2,thom1}. In spherical tokamaks, this is the only feasible option, because ECE tends to be optically thin, and
also encounters problems with cutoffs \cite{nstx1}.  The main drawback of Thomson scattering is that it does not have the same temporal resolution as
ECE radiometry. 
...
\funding{This research was directly funded by the U.S.\ Department of Energy, Office of Science, Office of Fusion Energy Sciences, under  
contract DE-FG02-04ER54742.}
...
It follows that $a\ll 1$ can be interpreted as  the mean inverse aspect-ratio of the plasma.
...
Moreover, an expansion of the Grad-Shafranov equation yields \cite{exp}, 
\begin{align}
...
flux-surface functions $q(\hat{r})$ and $p_2(\hat{r})$, the values of $H_2(1)$ and $H_3(1)$, and the equilibrium electron number density profile, $n_e(\hat{r})$. 
...
the first term in a Taylor expansion. The next term in the expansion  is of order $(q''\,W/q')_{r_l}\,X^{3}$. This term, which would lead to an additional source of
...
\end{align}
(The angle $\xi$ should not be confused with the plasma displacement $\bxi$.)
...
\xi_{m_{l}}^r(r_{l}+x)={\mit\Psi}\left(\frac{q}{r\,g}\,\frac{\hat{\psi}_{m_{l}}}{m_{l}-n\,q}\right)_{r_l+x}
...
\subsection{Results}\label{rotate}
...
$\zeta$ (which, in practice, is achieved by not modulating the source) \cite{ece6}. 

Incidentally, we are assuming that the NTM island chain is rotating, as is generally the case in present-day experiments. In Ref.~\cite{eddy},
it is argued that eddy currents excited in the ITER blanket module will cause a growing and rotating NTM to slow down, and eventually lock to a stray error-field,
when its associated island width exceeds a threshold value that is about $4.5\%$ of the plasma minor radius. This threshold island width for locking is much
smaller than that observed on present-day tokamaks. However, in Ref.~\cite{eddy1}, a more advanced calculation of island locking is performed that
incorporates the most up-to-date and comprehensive available data on the resistive eigenmodes of the ITER blanket modules and vacuum vessel. The
conclusion of this second calculation is that the critical island width for locking in ITER is 9\% of the plasma minor radius. Ideally, we would hope
to be able to detect an NTM by means of ECE, and take preventative action by means of ECCD,  long before its associated island width became as large as 4.5\% of the minor radius. Hence, the assumption, made in this paper,  that the NTM is rotating seems reasonable.
...
\omega_c = \frac{e\,B}{m_e}.
\end{equation}
Here, $\omega$ is the angular frequency of the emitted radiation, $e$ the magnitude of the electron charge, $B$ the (unnormalized) equilibrium magnetic field-strength at the emission site, and $\omega_c$  the local (non-relativistic) cyclotron frequency. 
...
\exp\left(-\frac{\left[R-R_\omega+{\mit\Delta}_1^{\,(O)}\right]^2}{2\left[\sigma_1^{\,(O)}\right]^2}\right)&R\leq R_\omega\nonumber\\[0.5ex]
&= 0& R>R_\omega,
\end{align}
where 
...
\end{align}
Here, $v_t$ and $\omega_p$ are evaluated at $z_2=0$. Moreover, the parameters $a$ (which should not be confused with the plasma minor radius), $b$, and $N_c$ have been calculated assuming that $\omega=2\,\omega_c$. 
...
\exp\left(-\frac{\left[R-R_\omega+{\mit\Delta}_2^{\,(X)}\right]^2}{2\left[\sigma_2^{\,(X)}\right]^2}\right)&R\leq R_\omega\nonumber\\[0.5ex]
&= 0& R>R_\omega.
\end{align}
...
\bibitem{tear10} R.~Fitzpatrick,  Phys.\ Plasmas {\bf 32}, 062509 (2025).
...
\bibitem{thom2} T.~O’Gorman,  G.~Naylor,  K.J.~Gibson,  B.~Huang, G.J.~McArdle, R.~Scannell,  S.~Shibaev, J.A.~Snape and N.~Thomas-Davies,
Rev.\ Sci.\ Instrum.\ {\bf 83}, 10E312 (2012).

\bibitem{thom1} L.~Bard\'{o}czi, T.A.~Carter, R.J.~La\,Haye, T.L.~Rhodes and G.R.~McKee, Phys.\ Plasmas {\bf 24}, 122503 (2017).

\bibitem{nstx1} P.C.~Efthimion, J.C.~Hosea, R.~Kaita, R.~Majeski and G.~Taylor, Rev.\ Sci.\ Instrum.\ {\bf 70} 1018 (1999). 
...
\bibitem{eddy} R.J.~La Haye, C.~Paz-Soldan and Y.Q.~Liu, Nucl.\ Fusion {\bf 57}, 014004 (2017).

\bibitem{eddy1} R.~Fitzpatrick, Phys.\ Plasmas {\bf 30}, 042514 (2023). 
...
\hline\\[-2.5ex]
$m$ & $n$ & $\hat{r}_l$ & $L_s$ & $E_{ll}$ & ${\mit\Delta}_{l\,{\rm crit}}$ & $\hat{\beta}$ & $\alpha_b$ & $\alpha_c$\ & $W_d/a$\\[0.5ex]\hline
~3~&~2~ & ~$0.6195$~ & ~$1.756$~ & ~$1.709$~ & ~$48.57$~ & ~$0.0540$~ & ~$2.588$~ & ~$0.5816$~&~$6.27\times 10^{-3}$\\[0.5ex]
2& 1& $0.7874$  & ~$1.209$~&$ 13.66$ & $47.96$ & $0.0293$ & $3.061$ & $1.000$ & $7.48\times 10^{-3}$\\\hline
\end{tabular}
\caption{The poloidal mode number, toroidal mode number, normalized minor radius, normalized magnetic shear-length, linear tearing stability index, critical linear tearing stability index for instability, normalized
electron pressure, bootstrap parameter, curvature parameter, and critical island width needed for flattening of the electron temperature, respectively, at the $3$, $2$ and the $2$, $1$ rational surfaces of the example
