...
%DIF DEL ../Paper/Separatrix.tex   Mon Nov 24 11:51:48 2025
%DIF ADD Separatrix.tex            Fri Feb  6 11:56:16 2026
...
%DIF 8a8-10
\newcommand {\bPsi}{\mbox{\boldmath$\Psi$}} %DIF > 
\newcommand {\bDelta}{\mbox{\boldmath$\Delta$}} %DIF > 
\newcommand {\bchi}{\mbox{\boldmath$\chi$}} %DIF > 
%DIF -------
\pdfoutput = 1 
\allowdisplaybreaks
...
\DIFdelbegin \DIFdel{It is estimated that $\epsilon_c=1.5\times 10^{-3}$ for an $n=1$
RMP, and $\epsilon_c=4.8\times 10^{-3}$ for an $n=4$ RMP,  in  a typical JET H-mode plasma. 
...
\DIFdelbegin \subsection{\DIFdel{Introduction}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend The aim of \DIFdelbegin \DIFdel{this section }\DIFdelend \DIFaddbegin \DIFadd{Sect.~\ref{stwo} }\DIFaddend is to construct a very simple model of a magnetically diverted tokamak plasma.
...
true toroidal geometry that takes place in Sect.~\ref{equil}.) Once the choice of toroidal angle is made, the \DIFaddbegin \DIFadd{straight-field-line }\DIFaddend coordinate system is uniquely defined. 
...
\DIFdelbegin \DIFdel{Equation}\DIFdelend \DIFaddbegin \DIFadd{A unit vector that runs around a magnetic flux-surface  in the $X$-$Y$ plane in the direction of increasing $\theta$ is such that
}\begin{equation}
{\DIFadd{\bf n}} \DIFadd{= \frac{\hat{\nabla}\phi\times\hat{\nabla}\psi}{|\hat{\nabla}\phi|\,|\hat{\nabla}\psi|}
}\end{equation}
\DIFadd{because ${\bf n} \cdot\hat{\nabla}\psi=0$ within a magnetic flux-surface,  ${\bf n} \cdot\hat{\nabla}\phi=0$ for a path confined to the $X$-$Y$ plane, and $\hat{\nabla}\phi\cdot\hat{\nabla}\psi=0$. 
Thus, 
Eq.}\DIFaddend ~(\ref{ex})  yields
\begin{equation}
\frac{d\theta}{dL} \DIFaddbegin \DIFadd{\equiv }{\bf \DIFadd{n}}\DIFadd{\cdot}\hat{\nabla}\DIFadd{\theta }\DIFaddend = \frac{q_\ast}{q\,|\hat{\nabla}\psi|}
\DIFdelbegin \DIFdel{.
}\DIFdelend \end{equation}
where $dL$ is an element of normalized length (in the $X$-$Y$ plane) around \DIFdelbegin \DIFdel{a magnetic }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend flux-surface\DIFaddbegin \DIFadd{, and use has been made of $|\hat{\nabla}\phi|=a/R_0$}\DIFaddend . 
It follows that
\begin{equation}\DIFaddbegin \label{e10}
\DIFaddend q(\psi) = \frac{q_\ast}{2\pi}\oint\frac{dL}{|\hat{\nabla}\psi|},
\end{equation}
...
However,   $q\rightarrow\infty$ as ${\mit\Psi}\rightarrow 1$.
 ...
 \end{equation}
 for ${\mit\Psi}>1$. 
 Moreover, the figure implies that $\alpha_+>\alpha_-$. \DIFaddbegin \DIFadd{The reason that $\alpha_+$ exceeds $\alpha_-$ is because magnetic field-lines lying in flux-surfaces situated just outside the X-point have
...
\DIFdelbegin \DIFdel{To }\DIFdelend \DIFaddbegin \DIFadd{In order to }\DIFaddend understand the significance of the straight-field-line coordinate system, suppose that the plasma is subject to a static (in the laboratory frame) magnetic perturbation that
...
electron fluid equation of motion yields \DIFaddbegin \DIFadd{(see Sect.~\ref{appa})
}\DIFaddend \begin{equation}\label{e21}
 n\,(\omega_E+\omega_{\ast\,e})\left[b^{\psi_p} -\DIFaddbegin {\rm \DIFadd{i}}\DIFadd{\,}\DIFaddend \frac{B_0}{R_0}\left(\frac{m}{q}-n\right)\xi^{\psi_p}\right] \simeq \frac{B_0}{R_0}\,\frac{m}{q} \,\eta_\parallel \,j_\phi,
\end{equation}
...
\begin{align}\DIFaddbegin \label{omegaE}
\DIFaddend \omega_E(\psi_p) = -\frac{d{\mit\Phi}}{d\psi_p}
\end{align}
is the {\em E-cross-B frequency}, whereas
\begin{align}\DIFaddbegin \label{omegae}
\DIFaddend \omega_{\ast\,e}(\psi_p) = \frac{1}{e\,n_e}\,\frac{dp_e}{d\psi_p}
\end{align}
is the {\em electron diamagnetic frequency}. 
Here, ${\mit\Phi}(\psi_p)$ is the \DIFaddbegin \DIFadd{equilibrium }\DIFaddend electrostatic potential,  $p_e(\psi_p)$  the \DIFaddbegin \DIFadd{equilibrium }\DIFaddend electron pressure, $n_e(\psi_p)$ the \DIFaddbegin \DIFadd{equilibrium }\DIFaddend electron number density, and $e$ the magnitude of the electron charge. 
...
\subsection{Effect of divertor plate}\DIFaddbegin \label{s2.8}
\DIFaddend Rational magnetic flux-surfaces exist outside the magnetic separatrix  \cite{reiman} because flux-surfaces in this region form closed loops, that complete below the
...
\caption{The magnetic flux-surfaces ${\mit\Psi}=0.9$ (red), ${\mit\Psi}=1.0$ (green), and ${\mit\Psi}=1.1$ (blue)  in the presence of a divertor plate located halfway between the X-point and the divertor coil filament. The black dot shows the location of plasma current filament, and the horizontal black line shows the location of the divertor plate. Here, $q_\ast=12$ \DIFdelbeginFL \DIFdelFL{amd }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL $\zeta=0.2$. }\label{fig5}
\end{figure}
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Introduction}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend The model described in Sect.~\ref{stwo} has the unrealistic feature that $q=0$ at the magnetic axis. This feature is
an artifact of treating the plasma current as a filament, rather than a spatially extended distribution. In \DIFdelbegin \DIFdel{this
section}\DIFdelend \DIFaddbegin \DIFadd{Sect.~\ref{sthree}}\DIFaddend , we shall attempt to construct an improved model.
...
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend The aim of \DIFdelbegin \DIFdel{this section is }\DIFdelend \DIFaddbegin \DIFadd{Sect.~\ref{response} is to }\DIFaddend analyze  the response of a tokamak plasma possessing a magnetic separatrix to a static RMP of toroidal mode number $n$. 
...
harmonic, but multiple  poloidal harmonics, characterized by the set of integer poloidal mode numbers $m$. As is well known, this is the case because, \DIFdelbegin \DIFdel{while
}\DIFdelend \DIFaddbegin \DIFadd{whereas 
}\DIFaddend different toroidal harmonics are not coupled together in an axisymmetric plasma equilibrium, different poloidal
...
linearized equations of \DIFdelbegin \DIFdel{marginally-stable, }\DIFdelend \DIFaddbegin \DIFadd{inertia-free (because the perturbation is static, and the plasma equilibrium remains an equilibrium in the presence of the perturbation) }\DIFaddend ideal-magnetohydrodynamics (MHD) \cite{fkr}. These equations reduce to
...
\end{align}
Here, the (dimensionless) $Z_m(r)$ functions are related to the \DIFaddbegin \DIFadd{poloidal harmonics of the }\DIFaddend perturbed toroidal magnetic field. Moreover, the (dimensionless) $L_m^{\,m'}(r)$, $M_m^{\,m'}(r)$, $N_m^{\,m'}(r)$,
...
system \cite{rf3}. \DIFaddbegin \DIFadd{In particular, $L_m^m(r)$ controls the radial  wavelength of the $m$, $n$ tearing mode, $P_m^m(r)$ encodes the destabilizing effect of equilibrium current and pressure gradients on the mode,
...
}\DIFaddend Note that Eqs.~(\ref{e47}) and (\ref{e48}) are singular at rational magnetic flux-surfaces at which $q=m/n$.
...
\DIFaddbegin \DIFadd{Moreover, $s$ is the magnetic shear, 
}\DIFaddend 
...
$k$th rational surface. \DIFaddbegin \DIFadd{The off-diagonal elements of $E_{kk'}$ specify the coupling between tearing modes that reconnect magnetic flux at different rational
surfaces. }\DIFaddend Finally, the $\chi_k$ specify the poloidal harmonics of the RMP  \cite{rf3,rf4}. 
...
\begin{equation}\DIFaddbegin \label{e62}
\DIFaddend {\mit\Psi}_k = \frac{\chi_k}{{\mit\Delta}_k - E_{kk}}.
\end{equation}
...
\DIFaddbegin \DIFadd{In reality, the plasma contains many rational surfaces. In this case, Eq.~(\ref{e62}) generalizes to give 
}\begin{equation}
\DIFadd{\bPsi = (\bDelta - }{\DIFadd{\bf E}}\DIFadd{)^{-1}\,\bchi,
}\end{equation}
\DIFadd{where $\bPsi$ is the vector of the ${\mit\Psi}_k$ values, $\bDelta$ the diagonal matrix of the ${\mit\Delta}_k$ values, ${\bf E}$ the matrix
of the $E_{kk'}$ values, and $\bchi$ the vector of the $\chi_k$ values. For the case of two rational surfaces, we get
}\begin{align}\DIFadd{\label{eff1}
}{\DIFadd{\mit\Psi}}\DIFadd{_1 }&\DIFadd{=\frac{({\mit\Delta}_2-E_{22})\,\chi_1+E_{12}\,\chi_2}{({\mit\Delta}_1-E_{11})\,({\mit\Delta}_2-E_{22})-E_{12}\,E_{21}},}\\[0.5ex]
{\DIFadd{\mit\Psi}}\DIFadd{_2 }&\DIFadd{=\frac{E_{21}\,\chi_1 + ({\mit\Delta}_1-E_{11})\,\chi_2}{({\mit\Delta}_1-E_{11})\,({\mit\Delta}_2-E_{22})-E_{12}\,E_{21}}.\label{eff2}
}\end{align}
\DIFadd{Now, we expect rational surfaces close to the magnetic separatrix to have large associated poloidal mode numbers. In the cylindrical limit, $E_{kk} = -2\,m_k$. 
In toroidal geometry, $E_{kk}$ differs somewhat from $-2\,m_k$. However, as can be demonstrated from toroidal tearing mode codes such as TJ \mbox{%DIFAUXCMD
\cite{rf3}}\hskip0pt%DIFAUXCMD
, it is still the case that $E_{kk}$ is large and negative, and $E_{kk} \propto -m_k$. In other words, it remains true that $E_{kk}\sim -2\,m_k$. Moreover, the magnitudes of the off-diagonal elements of the E-matrix also scale as $m_k$. 
...
}\begin{align}\DIFadd{\label{eax1}
}{\DIFadd{\mit\Psi}}\DIFadd{_1}&\DIFadd{\simeq \frac{\chi_1}{{\mit\Delta}_1},}\\[0.5ex]
{\DIFadd{\mit\Psi}}\DIFadd{_2}&\DIFadd{\simeq \frac{\chi_2}{{\mit\Delta}_2}.\label{eax2}
}\end{align}
\DIFadd{On the other hand, if  $|{\mit\Delta}_1|$, $|{\mit\Delta}_2| \ll |E_{11}|$, $|E_{22}|$, $|E_{12}|$, $|E_{21}|\sim 2\,m_1$ then we get
}\begin{align}
{\DIFadd{\mit\Psi}}\DIFadd{_1}&\DIFadd{\simeq \frac{-E_{22}\,\chi_1+E_{12}\,\chi_2}{E_{11}\,E_{12}-E_{12}\,E_{21}},\label{eax}}\\[0.5ex]
{\DIFadd{\mit\Psi}}\DIFadd{_2}&\DIFadd{\simeq \frac{E_{21}\,\chi_1 - E_{11}\,\chi_2}{E_{11}\,E_{12}-E_{12}\,E_{21}}.\label{eay}
}\end{align}
\DIFadd{Equations~(\ref{eax}) and (\ref{eay}) correspond to a vacuum response, in which the reconnected flux driven at the rational surfaces is the same
...
\DIFaddend \section{Behavior close to magnetic separatrix}\label{separatrix}
\DIFdelbegin \subsection{\DIFdel{Introduction}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend The aim of \DIFdelbegin \DIFdel{this section }\DIFdelend \DIFaddbegin \DIFadd{Sect.~\ref{separatrix} }\DIFaddend is to investigate asymptotic matching in the immediate vicinity of the magnetic separatrix. 
...
S &= \frac{\tau_R}{\tau_A}.
\end{align}
Here, $Z$ is the effective ion charge number, $T_e(r)$  the \DIFaddbegin \DIFadd{equilibrium }\DIFaddend electron temperature, $T_i(r)$  the \DIFaddbegin \DIFadd{equilibrium  }\DIFaddend ion temperature, $m_e$ the electron mass,  
...
\end{equation}
is the {\em ion diamagnetic frequency}. Moreover, $p_i(r)$ is the \DIFaddbegin \DIFadd{equilibrium }\DIFaddend ion pressure, and $\hat{d}_\beta=d_\beta/r$. As before, all quantities are evaluated at radius $r_k$. 
\DIFaddbegin \DIFadd{Note that $Q_E$ is the normalized ${\bf E}\times {\bf B}$ frequency, $Q_e$ the normalized electron diamagnetic frequency, $Q_i$ the normalized ion diamagnetic frequency, 
...
larger than the other parameters (e.g., $Q_E$, $Q_e$, $P_E$, $D$) that appear in Eqs.~(\ref{e80})--(\ref{e83}) \DIFaddbegin \DIFadd{(see Fig.~\ref{fig8a}) }\DIFaddend then it is clear that the first term on the left-hand side of Eq.~(\ref{e80}) dominates the second term in the region  $p\lesssim1$. In this case, we can integrate  the equation to give
\begin{equation}
...
\subsection{Plasma parameters}
 ...
 \begin{figure}[t]
\DIFdelbeginFL %DIFDELCMD < \centerline{\includegraphics[width=0.6\textwidth]{Figure9.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\includegraphics[width=0.9\textwidth]{Figure8a.pdf}}
\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Overlap criterion }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Normalized parameters }\DIFaddendFL for the resistive layers associated with an $n=1$ RMP close to the magnetic
separatrix of the JET plasma shown in Fig.~\ref{fig8}.\DIFdelbeginFL \DIFdelFL{The bold vertical dotted lines show the location of the magnetic separatrix, whereas
the faint vertical dotted lines indicate the boundaries of the region of resistive layer overlap.}\DIFdelendFL }\DIFdelbeginFL %DIFDELCMD < \label{fig9}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig8a}
...
Let us suppose that $\omega_E+ \omega_{\,\ast\,e}\simeq \omega_{\ast\,e}$ close to the magnetic separatrix. This is a reasonable assumption because we have no reason to imagine that the magnitude of $\omega_E$ greatly exceeds that of $\omega_{\ast\,e}$
 ...
 We can complete our model by adopting the following \DIFdelbegin \DIFdel{JET-like }\DIFdelend values of the remaining unspecified plasma parameters: \DIFdelbegin \DIFdel{$B_0=3.45\,{\rm T}$}\DIFdelend \DIFaddbegin \DIFadd{$B_0=2.0\,{\rm T}$}\DIFaddend , $R_0=2.96\,{\rm m}$, \DIFdelbegin \DIFdel{$a=1.25\,{\rm m}$}\DIFdelend \DIFaddbegin \DIFadd{$a=0.9\,{\rm m}$}\DIFaddend , $Z=10.0$,
 $M=2.0$, $\chi_E=\chi_\phi=1\,{\rm m^{2}/s}$ \cite{jetx}. \DIFaddbegin \DIFadd{(These values are consistent with those of JET discharge 84600.) }\DIFaddend Here, $M$ is the ion mass number, and the rather large value of $Z$ is meant to account for the presence of a significant fraction
 of trapped particles, in addition to plasma impurities, close to the plasma boundary. 

 \DIFaddbegin \DIFadd{Figure~\ref{fig8a} shows the normalized parameters for the resistive layers associated with an $n=1$ RMP close to the magnetic
...
\DIFdelbeginFL %DIFDELCMD < \centerline{\includegraphics[width=0.6\textwidth]{Figure10.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\includegraphics[width=0.6\textwidth]{Figure9.pdf}}
\DIFaddendFL \caption{Overlap criterion for the resistive layers associated with an \DIFdelbeginFL \DIFdelFL{$n=4$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$n=1$ }\DIFaddendFL RMP close to the magnetic
...
the faint vertical dotted lines indicate the boundaries of the region of resistive layer overlap.}\DIFdelbeginFL %DIFDELCMD < \label{fig10}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig9}
\DIFaddendFL \end{figure}
...
in the immediate vicinity of the separatrix. In fact, the region of overlap extends from \DIFdelbegin \DIFdel{${\mit\Psi}=0.9985$ to $1.0009$}\DIFdelend \DIFaddbegin \DIFadd{${\mit\Psi}=0.9980$ to $1.0013$}\DIFaddend . 
...
\caption{\DIFaddFL{Overlap criterion for the resistive layers associated with an $n=4$ RMP close to the magnetic
separatrix of the JET plasma shown in Fig.~\ref{fig8}. The bold vertical dotted lines show the location of the magnetic separatrix, whereas
the faint vertical dotted lines indicate the boundaries of the region of resistive layer overlap.}}\label{fig10}
\end{figure}

\DIFaddend The lower panel of Fig.~\ref{fig9} shows the ratio $|{\mit\Delta}_k|/(-E_{kk})$. Here, we have used the cylindrical result $E_{kk}=-2\,|m|$
...
extends from \DIFdelbegin \DIFdel{${\mit\Psi}= 0.9952$ to $1.0043$}\DIFdelend \DIFaddbegin \DIFadd{${\mit\Psi}= 0.9938$ to $1.0068$}\DIFaddend . Thus, the width  in ${\mit\Psi}$ of the overlap region exhibits a roughly linearly scaling with $n$. 
As before, the response of the plasma interior is ideal, whereas that of the overlap region and the  SOL could reasonably
be characterized as being vacuum-like\DIFdelbegin \DIFdel{,
...
\DIFdelbegin \DIFdel{Furthermore, }\DIFdelend \DIFaddbegin \DIFadd{The fact that the resistive layers overlap in the immediate vicinity of the magnetic separatrix indicates that this region acts like a giant resistive layer. In other words, all
three terms in the induction equation, (\ref{e21}), are of similar magnitude in the overlap region. Hence, flux-freezing is not enforced, and there is no real distinction between rational and irrational
magnetic flux-surfaces. The question remains as to whether or not a large shielding current flows in the overlap region. However, as is clear from the
previous analysis, the fact that $|{\mit\Delta}_k|/(-E_{kk})\sim {\cal O}(1)$ in this  region indicates that only weak shielding currents are excited. Thus, the
overlap region is a region in which flux-freezing is not enforced, and in which comparatively weak shielding currents are excited, which implies that the region is
essentially indistinguishable from a vacuum. 
In other words, }\DIFaddend while the plasma response in the plasma interior lies in the ideal regime, that in the region of resistive layer
...
only need to include rational surfaces whose resonant poloidal mode numbers are less than \DIFdelbegin \DIFdel{8. }\DIFdelend \DIFaddbegin \DIFadd{6. }\DIFaddend On the other hand, for an $n=4$ RMP, we  need to include all rational surfaces whose resonant poloidal mode numbers are less than \DIFdelbegin \DIFdel{24. 
}\DIFdelend \DIFaddbegin \DIFadd{22. 
}\DIFaddend Because the straight-field-line coordinate
system is only required in the region $0<{\mit\Psi}<1-\epsilon_c$,  where \DIFdelbegin \DIFdel{$\epsilon_c>1$}\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_c>0$}\DIFaddend ,  the fact that the system becomes singular at ${\mit\Psi}=1$ is irrelevant. Indeed, we are
...
linearized,  \DIFdelbegin \DIFdel{marginally-stable}\DIFdelend \DIFaddbegin \DIFadd{inertia-free}\DIFaddend , ideal-MHD. However, these equations are singular at the various rational surfaces in the plasma. 
...
\DIFaddFL{$Z$ }& \DIFaddFL{$\epsilon_c$ }&\DIFaddFL{$Z$ }& \DIFaddFL{$\epsilon_c$}\\\hline
&&&\\[-2ex]
\DIFaddFL{$1$ }& \DIFaddFL{$6.86\times 10^{-4}$ }& \DIFaddFL{$6$ }& \DIFaddFL{$1.58\times10^{-3}$}\\
\DIFaddFL{$2$ }& \DIFaddFL{$9.49\times 10^{-4}$ }& \DIFaddFL{$7$ }& \DIFaddFL{$1.69\times10^{-3}$}\\
\DIFaddFL{$3$ }& \DIFaddFL{$1.15\times 10^{-3}$ }& \DIFaddFL{$8$ }& \DIFaddFL{$1.80\times10^{-3}$}\\
\DIFaddFL{$4$ }& \DIFaddFL{$1.31\times 10^{-3}$ }& \DIFaddFL{$9$ }& \DIFaddFL{$1.89\times10^{-3}$}\\
\DIFaddFL{$5$ }& \DIFaddFL{$ 1.45\times 10^{-3}$ }& \DIFaddFL{$10$ }& \DIFaddFL{$1.98\times10^{-3}$
}\end{tabular}
\caption{\DIFaddFL{Variation of $\epsilon_c$ with $Z$ for an $n=1$ RMP in a typical JET H-mode plasma.}}\label{t1}
\end{table}

\DIFaddend In Sect.~\ref{separatrix}, we examine the asymptotic matching problem close to the magnetic separatrix. We find that, due to the strong magnetic
...
edge plasma parameters. \DIFdelbegin \DIFdel{We find that $\epsilon_c=1.5\times 10^{-3} $ }\DIFdelend \DIFaddbegin \DIFadd{Assuming that the effective ion charge number takes the value $Z=10$, we find that $\epsilon_c=2.0\times 10^{-3} $ }\DIFaddend for an $n=1$
RMP,  and \DIFdelbegin \DIFdel{$\epsilon_c=4.8\times 10^{-3}$ }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon_c=6.2\times 10^{-3}$ }\DIFaddend for an $n=4$ RMP,  in  a typical JET H-mode plasma. \DIFaddbegin \DIFadd{Table~\ref{t1} shows how $\epsilon_c$ varies with $Z$ in the
...
%DIF < This research was funded by the  U.S.\ Department of Energy, Office of Science, Office of Fusion Energy Sciences under contract DE-FG02-04ER54742.

%DIF < \section*{Data availability statement}
\data{The digital data used in the figures in this paper can be obtained from the author upon reasonable request.}
%DIF < The digital data used in the figures in this paper can be obtained from the author upon reasonable request.

\DIFaddbegin \appendix
\section{\DIFadd{Derivation of Induction Equation}}\label{appa}
\DIFadd{The equilibrium magnetic field (\ref{e2}) can be written
}\begin{equation}\DIFadd{\label{bdef}
}{\DIFadd{\bf B}} \DIFadd{= }{\DIFadd{\bf B}}\DIFadd{_p + }{\DIFadd{\bf B}}\DIFadd{_t= \nabla\phi\times \nabla\psi_p-q\,\nabla\theta\times \nabla\psi_p,
}\end{equation}
\DIFadd{where ${\bf B}_p$ and ${\bf B}_t$ are the ``poloidal'' and ``toroidal'' components of the field. 
According to Eq.~(\ref{e3}), the Jacobian of the $\psi_p$, $\theta$, $\phi$ coordinate system is
}\begin{equation}\DIFadd{\label{e104}
}{\DIFadd{\cal J}} \DIFadd{\equiv (\nabla\psi_p\times\nabla\theta\cdot\nabla\phi)^{-1}=\frac{R_0\,q}{B_0}.
}\end{equation}
\DIFadd{It follows that
}\begin{align}\DIFadd{\label{e105}
B^{\psi_p} }&\DIFadd{\equiv }{\DIFadd{\bf B}}\DIFadd{\cdot\nabla\psi_p = 0,}\\[0.5ex]
\DIFadd{B^\theta }&\DIFadd{\equiv{\bf B}\cdot \nabla\theta= }{\DIFadd{\cal J}}\DIFadd{^{-1},}\\[0.5ex]
\DIFadd{B^\phi }&\DIFadd{\equiv }{\DIFadd{\bf B}}\DIFadd{\cdot\nabla\phi = }{\DIFadd{\cal J}}\DIFadd{^{-1}\,q.\label{e107}
}\end{align}
\DIFadd{Here, and in the following, subscripts and superscripts denote covariant and contravariant components in the $\psi_p$, $\theta$, $\phi$ coordinate system.
}

\DIFadd{The equilibrium electron fluid velocity takes the conventional form \mbox{%DIFAUXCMD
\cite{book}
}\hskip0pt%DIFAUXCMD
}\begin{equation}
{\DIFadd{\bf V}}\DIFadd{_e = \frac{{\bf E}\times {\bf B}}{B^2} + \frac{\nabla p_e\times {\bf B}}{e\,n_e\,B^2},
}\end{equation}
...
\DIFadd{V_e^{\psi_p} }&\DIFadd{\equiv }{\DIFadd{\bf V}}\DIFadd{_e\cdot\nabla\psi_p=0,\label{vepp}}\\[0.5ex]
\DIFadd{V_e^\theta}& \DIFadd{\equiv }{\DIFadd{\bf V}}\DIFadd{_e\cdot\nabla\theta=- \frac{(\omega_E+\omega_{\ast\,e})}{q}\,\frac{B_t^{\,2}}{B^2},\label{vet}}\\[0.5ex]
\DIFadd{V_e^\phi}&\DIFadd{\equiv }{\DIFadd{\bf V}}\DIFadd{_e\cdot\nabla\phi = (\omega_E+\omega_{\ast\,e})\,\frac{B_p^{\,2}}{B^2},\label{vep}
}\end{align}
\DIFadd{where $\omega_E$ and $\omega_{\ast\,e}$ are defined in Eqs.~(\ref{omegaE}) and (\ref{omegae}), respectively, and use has been made of Eq.~(\ref{bdef}). 
}

\DIFadd{Let ${\bf v}_e$ be the perturbed electron fluid velocity. Let us define the (steady-state) component of the Lagrangian electron fluid displacement that is contravariant to $\psi_p$ such that
}\begin{align}
\DIFadd{v_e^{\psi_p} }&\DIFadd{= }{\DIFadd{\bf V}}\DIFadd{_e\cdot\nabla\xi^{\psi_p} = \left(V_e^\theta\,\frac{\partial}{\partial\theta} + V_e^\phi\,\frac{\partial}{\partial\phi}\right)\xi^{\psi_p}
= }{\DIFadd{\rm i}}\DIFadd{\left(m\,V_e^\theta-n\,V_e^\phi\right)\xi^{\psi_p}\nonumber}\\[0.5ex]
&\DIFadd{= -}{\DIFadd{\rm i}}\DIFadd{\,(\omega_E+\omega_{\ast\,e})\left(\frac{m\,B_t^{\,2}/q+n\,B_p^{\,2}}{B^2}\right)\xi^{\psi_p}\simeq -}{\DIFadd{\rm i}}\DIFadd{\,n\,(\omega_E+\omega_{\ast\,e})\,\xi^{\psi_p}.\label{112}
}\end{align}
\DIFadd{Here, use has been made of Eqs.~(\ref{vet}) and (\ref{vep}), as well as the assumption that we are relatively close to the rational surface at which $q=m/n$. 
}

\DIFadd{The electron fluid equation of motion takes the form \mbox{%DIFAUXCMD
\cite{book}
}\hskip0pt%DIFAUXCMD
}\begin{equation}
\DIFadd{\frac{\nabla p_e}{e\,n_e} + }{\DIFadd{\bf E}} \DIFadd{+ }{\DIFadd{\bf V}}\DIFadd{_e\times }{\DIFadd{\bf B}} \DIFadd{= \eta_\parallel\,{\bf J},
}\end{equation}
\DIFadd{where $\eta_\parallel$ is the parallel electrical resistivity,  and ${\bf J}$ the current density.  Note that we are neglecting the anisotropy in the plasma resistivity (as well as the thermal force), for the sake of simplicity. 
...
}\begin{equation}\DIFadd{\label{e114}
\nabla\times (}{\DIFadd{\bf v}}\DIFadd{_e\times }{\DIFadd{\bf B}}\DIFadd{) +\nabla\times (}{\DIFadd{\bf V}}\DIFadd{_e\times{\bf b}) = \nabla\times (\eta_\parallel\,{\bf j}),
}\end{equation}
\DIFadd{where ${\bf b}$ is the perturbed magnetic field, and ${\bf j}$ the perturbed current density. Here, we have assumed that $\nabla\cdot{\bf V}_e\simeq \nabla\cdot{\bf v}_e\simeq 0$. 
Now \mbox{%DIFAUXCMD
\cite{rf3}}\hskip0pt%DIFAUXCMD
,
}\begin{align}
\DIFadd{(}{\DIFadd{\bf v}}\DIFadd{_e\times{\bf B})_\theta }&\DIFadd{= }{\DIFadd{\cal J}}\DIFadd{\,(v_e^{\phi}\,B^{\psi_p}-v_e^{\psi_p}\,B^\phi)=-q\, v_e^{\psi_p} ,}\\[0.5ex]
\DIFadd{(}{\DIFadd{\bf v}}\DIFadd{_e\times }{\DIFadd{\bf B}}\DIFadd{)_\phi}&\DIFadd{=}{\DIFadd{\cal J}}\DIFadd{\,(v_e^{\psi_p}\,B^\theta-v_e^\theta\,B^{\psi_p})= v_e^{\psi_p},}\\[0.5ex]
{\DIFadd{\cal J}} \DIFadd{\,[\nabla\times({\bf v}_e\times{\bf B})]^{\psi_p} }&\DIFadd{= \frac{\partial}{\partial\theta}\,(}{\DIFadd{\bf v}}\DIFadd{_e\times }{\DIFadd{\bf B}}\DIFadd{)_\phi -\frac{\partial}{\partial\phi}\,(}{\DIFadd{\bf v}}\DIFadd{_e\times }{\DIFadd{\bf B}}\DIFadd{)_\theta\nonumber}\\[0.5ex]
&\DIFadd{= }{\DIFadd{\rm i}}\DIFadd{\,(m-n\,q)\,v_e^{\psi_p}= n\,(\omega_E+\omega_{\ast\,e})\,(m-n\,q)\,\xi^{\psi_p},\label{e117}}\\[0.5ex]
\DIFadd{(}{\DIFadd{\bf V}}\DIFadd{_e\times{\bf b})_\theta }&\DIFadd{= }{\DIFadd{\cal J}}\DIFadd{\,(V_e^{\phi}\,b^{\psi_p}-V_e^{\psi_p}\,b^\phi)= }{\DIFadd{\cal J}}\DIFadd{\,(\omega_E+\omega_{\ast\,e})\,\frac{B_p^{\,2}}{B^2}\,b^{\psi_p},}\\[0.5ex]
\DIFadd{(}{\DIFadd{\bf V}}\DIFadd{_e\times }{\DIFadd{\bf b}}\DIFadd{)_\phi}&\DIFadd{= }{\DIFadd{\cal J}}\DIFadd{\,(V_e^{\psi_p}\,b^\theta-V_e^\theta\,b^{\psi_p})=\frac{{\cal J}\,(\omega_E+\omega_{\ast\,e})}{q}\,\frac{B_t^{\,2}}{B^2}\,b^{\psi_p},}\\[0.5ex]
{\DIFadd{\cal J}} \DIFadd{\,[\nabla\times({\bf V}_e\times{\bf b})]^{\psi_p} }&\DIFadd{= \frac{\partial}{\partial\theta}\,(}{\DIFadd{\bf V}}\DIFadd{_e\times }{\DIFadd{\bf b}}\DIFadd{)_\phi -\frac{\partial}{\partial\phi}\,(}{\DIFadd{\bf V}}\DIFadd{_e\times }{\DIFadd{\bf b}}\DIFadd{)_\theta\nonumber}\\[0.5ex]
&\DIFadd{\simeq }{\DIFadd{\rm i}}\DIFadd{\,{\cal J}\,(\omega_E+\omega_{\ast\,e})\left(\frac{m}{q}\,\frac{B_t^{\,2}}{B^2}+n\,\frac{B_p^{\,2}}{B^2}\right)b^{\psi}_p\simeq }{\DIFadd{\rm i}}\DIFadd{\,n\,{\cal J}\,(\omega_E+\omega_{\ast\,e})\,b^{\psi_p},\label{e119}
}\end{align}
\DIFadd{where use has been made of Eqs.~(\ref{e105})--(\ref{e107}) and (\ref{vepp})--(\ref{112}), as well as the fact that $B_t^{\,2}$ only depends weakly on $\theta$ in a large aspect-ratio tokamak, and the assumption that $m\simeq n\,q$. 
}

\DIFadd{Now,
}\begin{equation}
{\DIFadd{\cal J}}\DIFadd{\,[\nabla\times (\eta_\parallel\,{\bf j})]^{\psi_p} = \frac{\partial}{\partial\theta}\,(\eta_\parallel\,j_\phi) - \frac{\partial}{\partial\phi}\,(\eta_\parallel\,j_\theta)\simeq }{\DIFadd{\rm i}}\DIFadd{\,m\,\eta_\parallel\,j_\phi,
}\end{equation}
\DIFadd{because $|j_\theta|/|j_\phi|\simeq (a/R_0)\,(B_p/B_t)\ll 1$ in a large aspect-ratio tokamak, assuming that the perturbed current flows predominately parallel to equilibrium magnetic field-lines. Thus,  the component of
Eq.~(\ref{e114}) that is contravariant to $\psi_p$ yields
}\begin{equation}
\DIFadd{n\,(\omega_E+\omega_{\ast\,e})\left[b^{\psi_p} - }{\DIFadd{\rm i}}\DIFadd{\,\frac{R_0}{B_0}\left(\frac{m}{q}-n\right)\xi^{\psi_p}\right] = \frac{B_0}{R_0}\,\frac{m}{q}\,\eta_\parallel\,j_\phi,
}\end{equation}
\DIFadd{where use has been made of Eqs.~(\ref{e104}), (\ref{e117}), and (\ref{e119}). 
}

\DIFaddend \begin{thebibliography}{99}\baselineskip 5ex
...
\bibitem{zheng} L.J.~Zheng, M.T.~Kotschenreuther and P.~Valanju, {\em The sensitivity of tokamak magnetohydrodynamics stability on the edge equilibrium},  Phys.\ Plasmas {\bf 24}, 102503 (2017\DIFaddbegin \DIFadd{).
}

\bibitem{book} \DIFadd{R.~Fitzpatrick, }{\em \DIFadd{Plasma Physics: An Introduction}}\DIFadd{, (CRC Press, Boca Raton FL, 2022}\DIFaddend ).
