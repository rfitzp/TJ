...
\author{Richard Fitzpatrick\,\footnote{rfitzp@utexas.edu}\\[0.5ex]{\em Institute for Fusion Studies, Department of Physics,}\\[0.5ex] {\em University of Texas at Austin, Austin TX 78712, USA}}
\date{}
...
magnetic perturbation (RMP), it is necessary to include an infinite number of rational magnetic flux-surfaces in the calculation, the
...
lie in the region $0<{\mit\Psi}<{\mit\Psi}_c$, where ${\mit\Psi}$ is the normalized poloidal magnetic flux, and
${\mit\Psi}_c<1$ can be calculated from the edge plasma parameters. It is estimated that ${\mit\Psi}_c= 0.9985$ for an $n=1$
RMP, and ${\mit\Psi}_c=0.9952$ for an $n=4$ RMP,  in  a typical JET H-mode plasma. 
\end{abstract}

\section{Introduction}
All modern tokamak plasmas feature a {\em magnetic divertor},  which is a particular configuration of the magnetic field that redirects the outermost field-lines  away from the confined plasma region and into a controlled exhaust area where the plasma particle and heat fluxes can safely be absorbed by a solid  target plate \cite{wesson}. Each magnetic flux-surface in a tokamak has an associated value of the {\em safety-factor}, which specifies the average number of
toroidal circuits of the surface that a constituent  magnetic field-line makes per poloidal circuit \cite{boozer}. Magnetic flux-surfaces possessing rational
...
topology, and are only partialy occupied by plasma. The LCFS features a magnetic ``X-point", which is a circular loop, coaxial with the plasma torus, on
...
safety-factor \cite{ipec,epec,gpec}. The purpose of this paper is to investigate to what extent this approach is justified. 
...
used to define exactly what is meant by a flux coordinate system, the safety-factor profile, and a rational surface, both inside and outside the LCFS. 
...
to an RMP is analyzed  in Sect.~\ref{response}. Section~\ref{separatrix}  investigates the behavior of the plasma
response model in the vicinity of the LCFS. Finally, the paper is summarized, and conclusions are drawn, in Sect.~\ref{sum}. 
...
\psi &= \frac{1}{2}\,\ln\left(X^2+Y^2\right) + \frac{\zeta}{2}\,\ln\left[X^2+(Y+1)^2\right],\\[0.5ex]
\psi_X &= \frac{X}{X^2+Y^2} + \frac{\zeta\,X}{X^2+(Y+1)^2},\\[0.5ex]
\psi_Y &= \frac{Y}{X^2+Y^2} +\frac{\zeta\,(Y+1)}{X^2+(Y+1)^2},
\end{align}
...
As is easily demonstrated, the coordinates of this point are ($X_c$, $Y_c$), where  $X_c=0$ and $Y_c=-1/(1+\zeta)$. 
The magnetic separatrix corresponds to the curve $\psi(X,Y)= \psi_c$, where 
\begin{equation}
\psi_c \equiv \psi(X_c,Y_c) = \ln\left[\frac{\zeta^{\,\zeta}}{(1+\zeta)^{1+\zeta}}\right].
\end{equation}
...
\end{equation}
where $dL$ is an element of normalized length (in the $X$-$Y$ plane) around a magnetic flux-surface. 
It follows that
\begin{equation}
...
To understand the significance of the flux coordinate system, suppose that the plasma is subject to a static (in the laboratory frame) magnetic perturbation that
...
\begin{equation}\label{e21}
 n\,(\omega_E+\omega_{\ast\,e})\left[b^{\psi_p} -\frac{B_0}{R_0\,q}\,(m-n\,q)\,\xi^{\psi_p}\right] \simeq \frac{B_0}{R_0\,q}\,m \,\eta_\parallel \,j_\phi,
\end{equation}
where subscripts and superscripts denote covariant and contravariant components in the $\psi_p$, $\theta$, $\phi$ coordinate
system, respectively.  Here, ${\bf b}$ is the perturbed magnetic field, $\bxi$ the Lagrangian electron fluid displacement,  ${\bf j}$ the perturbed current
...
Rational magnetic flux-surfaces exist outside the magnetic separatrix  because flux-surfaces in this region form closed loops, that complete below the
...
field-lines. However, within the divertor plate (which is assumed to be electrically conducting), the filament  takes the path of least electrical resistance, which implies that it runs along the
...
\subsection{Introduction}
The aim of this section is analyze  the response of a plasma possessing a magnetic separatrix to a static RMP of toroidal mode number $n$. 
...
linearized equations of marginally-stable, ideal-magnetohydrodynamics (MHD) \cite{fkr}. These equations reduce to
...
\end{align}
Here, the (dimensionless) $Z_m(r)$ functions are related to the perturbed toroidal magnetic field. Moreover, the (dimensionless) $L_m^{\,m'}(r)$, $M_m^{\,m'}(r)$, $N_m^{\,m'}(r)$,
and $P_m^{\,m'}(r)$ coefficients are determined by the equilibrium profiles and the metric elements of the $r$, $\theta$, $\phi$ flux coordinate
system \cite{rf3}. 
...
$k$th rational surface. Finally, the $\chi_k$ specify the poloidal harmonics of the RMP  \cite{rf3,rf4}. 
...
surfaces on which the constraints ${\mit\Psi}_k\simeq 0$ have to be imposed. On the other hand, in a region of the plasma in which the response lies in the vacuum
...
automatically satisfies the constraints that ${\mit\Delta\Psi}_k=0$ at the rational surfaces. Thus, there is no need for us to determine the locations of the
rational surfaces, which means that we do not have to employ a flux coordinate system, which implies that there is no reason for our
adopted coordinate system to become singular anywhere. 

\section{Behavior close to magnetic separatrix}\label{separatrix}
\subsection{Introduction}
The aim of this section is to investigate asymptotic matching in the immediate vicinity of the magnetic separatrix. 
...
\end{equation}
where $y=\hat{r}-1$.
Observe that the magnitude of the shear becomes infinite as the magnetic separatrix ($y=0$) is approached. 
...
$m_i$ the ion mass, $\chi_\perp(r)$ the perpendicular energy
...
where
\begin{align}
A &= \frac{(n\,s)^2\,p^2}{-{\rm i}\,(Q_E+Q_e)+p^2},\\[0.5ex]
B &= - {\rm i}\,Q_E\,(Q_E+Q_i)-{\rm i}\,(Q_E+Q_i)\,(P_\varphi+P_\perp)\,p^2 + P_\varphi\,P_\perp\,p^4,\\[0.5ex]
C &= - {\rm i}\,(Q_E+Q_e)+ \left[P_\perp-{\rm i}\,(Q_E+Q_i)\,D^{\,2}\right]p^2
...
\end{equation}
is the {\em ion diamagnetic frequency}. Moreover, $p_i(r)$ is the ion pressure, and $\hat{d}_\beta=d_\beta/r$. As before, all quantities are evaluated at radius $r_k$. 
...
larger than the other parameters (e.g., $Q_E$, $Q_e$, $P_\perp$, $D$) that appear in Eqs.~(\ref{e80})--(\ref{e83}) then it is clear that the first term on the left-hand side of Eq.~(\ref{e80}) dominates the second term in the region  $p\lesssim1$. In this case, we can integrate  the equation to give
\begin{equation}
...
\begin{equation}
\frac{d^2\overline{Y}}{d\hat{p}^{\,2}} -\frac{B}{C}\,\hat{p}^{\,2}\,\overline{Y}\simeq 0,
\end{equation}
where 
\begin{align}
B &= - {\rm i}\,Q_E\,(Q_E+Q_i)-{\rm i}\,(Q_E+Q_i)\,(P_\varphi+P_\perp)\,(n\,|s|)\,\hat{p}^{\,2} + P_\varphi\,P_\perp\,(n\,|s|)^2\,\hat{p}^{\,4}\nonumber\\[0.5ex]
&\simeq  P_\varphi\,P_\perp\,(n\,|s|)^2\,\hat{p}^{\,4},\\[0.5ex]
C &= - {\rm i}\,(Q_E+Q_e)+ \left[P_\perp-{\rm i}\,(Q_E+Q_i)\,D^{\,2}\right](n\,|s|)\,\hat{p}^{\,2}
...
\frac{n\,(\omega_E+\omega_{\ast\,e})\,\tau_A^{\,1/2}\,\tau_R^{\,3/4}}{\tau_\perp^{\,1/4}\,\hat{d}_\beta^{\,1/2}\,(n\,|s|)^{1/2}},
\end{equation}
...
\begin{equation}
\hat{\delta}_k(\hat{r}) = \frac{\tau_A^{\,1/2}}{\tau_R^{\,1/4}\,\tau_\perp^{\,1/4}\,\hat{d}_\beta^{\,1/2}\,(n\,|s|)^{1/2}}.
\end{equation}
...
\begin{figure}[t]
\centerline{\includegraphics[width=0.5\textwidth]{Figure8.pdf}}
\caption{Model safety-factor, electron temperature, and electron number density profiles at the edge of a typical JET  H-mode plasma. }\label{fig8}
\end{figure}
...
\begin{figure}[t]
\centerline{\includegraphics[width=0.5\textwidth]{Figure9.pdf}}
\caption{Overlap criterion for the resistive layers associated with an $n=1$ RMP close to the magnetic
separatrix of the JET plasma shown in Fig.~\ref{fig8}.}\label{fig9}
\end{figure}
...
Let us suppose that $\omega_E+ \omega_{\,\ast\,e}\simeq \omega_{\ast\,e}$ close to the magnetic separatrix. This is a reasonable assumption because we have no reason to imagine that the magnitude of $\omega_E$ greatly exceeds that of $\omega_{\ast\,e}$
 ...
 We can complete our model by adopting the following JET-like values of the remaining unspecified plasma parameters: $B_0=3.45\,{\rm T}$, $R_0=2.96\,{\rm m}$, $a=1.25\,{\rm m}$, $Z=10.0$,
 $M=2.0$, $\chi_\perp=\chi_\phi=1\,{\rm m^{2}/s}$ \cite{jetx}. Here, $M$ is the ion mass number, and the rather large value of $Z$ is meant to account for the presence of a significant fraction
 of trapped particles, in addition to plasma impurities, close to the plasma boundary. 

 \begin{figure}[t]
\centerline{\includegraphics[width=0.5\textwidth]{Figure10.pdf}}
\caption{Overlap criterion for the resistive layers associated with an $n=4$ RMP close to the magnetic
separatrix of the JET plasma shown in Fig.~\ref{fig8}.}\label{fig10}
\end{figure}
 
\subsection{Overlap criterion}
...
in the immediate vicinity of the separatrix. In fact, the figure indicates that the region of overlap extends from ${\mit\Psi}=0.9985$ to $1.0009$. 
...
It should be noted that the spacing between successive rational surfaces, $\hat{\epsilon}_k$, and the resistive layer width, $\hat{\delta}_k$, exhibit
no dependence on the angular coordinates  $\theta$ and $\phi$, when expressed in terms of the radial flux coordinate $\hat{r}$.
On the other hand, in the vicinity of the magnetic separatrix,  the  spacing and width in the $R$-$Z$ plane exhibit strong dependences on the angular coordinates. This behavior occurs because $|\nabla\hat{r}|\sim \ell\,\ln|{\mit\Psi}-1|$, where $\ell$ is the distance from the X-point in the $R$-$Z$ plane.
Nevertheless, two neighboring resistive layers that overlap at the mid-plane  ($Z=0$)  also overlap in the vicinity of the X-point, and vice versa. 
...
it would not be unreasonable to characterize the response of the overlap region and the  SOL as being vacuum-like. 
...
extends from ${\mit\Psi}= 0.9952$ to $1.0043$. Thus, the width  in ${\mit\Psi}$ of the overlap region exhibits a roughly linearly scaling with $n$. 
...
in this region, which implies that we must adopt a flux coordinate system. On the other hand, in the overlap region and the SOL, we can
write ${\bf b}=\nabla V$, where $\nabla^2 V=0$, which automatically ensures that the response is vacuum-like. In this case, there is
no need to know the locations of the rational surfaces, which implies that it is not necessary to adopt a flux coordinate system. 
...
magnetic separatrix lie at ${\mit\Psi}={\mit\Psi}_c$. Note that ${\mit\Psi}_c<1$. We must solve Eqs.~(\ref{e47}) and (\ref{e48}) in the region $0<{\mit\Psi}<{\mit\Psi}_c$, using
the flux coordinate system specified in Eq.~(\ref{fluxc}), and treating the region ${\mit\Psi}>{\mit\Psi}_c$ as a vacuum. Of course, this
is the approach taken in existing plasma response codes such as GPEC \cite{ipec}, in which ${\mit\Psi}_c$ is given an arbitrary value that lie just below unity, such as $0.998$. (This course of action is adopted because GPEC cannot deal with the singularity of the flux coordinate system at ${\mit\Psi}=1$, and
...
determine the correct value of ${\mit\Psi}_c$ from the edge plasma parameters, and also justifies the neglect of the plasma in the region ${\mit\Psi}>{\mit\Psi}_c$.  The fact that the region ${\mit\Psi}>{\mit\Psi}_c$ is treated as a vacuum
...
Because the flux coordinate
system is only required in the region $0<{\mit\Psi}<{\mit\Psi}_c$,  where ${\mit\Psi}_c<1$,  the fact that the system becomes singular at ${\mit\Psi}=1$ is irrelevant. Indeed, we are
free to choose a convenient non-singular coordinate system in the vacuum region, ${\mit\Psi}>{\mit\Psi}_c$. 
...
diverted tokamak plasma to an RMP. Essentially, we treat the region of the plasma that lies outside the inner boundary of the
overlap region, which lies at ${\mit\Psi}={\mit\Psi}_c$, where ${\mit\Psi}_c<1$,  as a vacuum. This means that  that rational surfaces that lie in the
region ${\mit\Psi}>{\mit\Psi}_c$ do not contribute to the plasma response. Hence, a plasma response calculation only needs to take a
finite number of rational surfaces into account. Our analysis enables us to determine ${\mit\Psi}_c$ from the 
edge plasma parameters. We find that ${\mit\Psi}_c= 0.9985$ for an $n=1$
RMP,  and ${\mit\Psi}_c=0.9952$ for an $n=4$ RMP,  in  a typical JET H-mode plasma. 
...
\bibitem{ipec} J.-K.~Park,  A.H.~Boozer and  A.H.~Glasser,  Phys.\ Plasmas {\bf 14}, 052110 (2007). 

\bibitem{epec} R.~Fitzpatrick, Phys.\ Plasmas {\bf 27}, 102511 (2020).

\bibitem{gpec} Q.M.~Hu, J.-K.~Park, N.C.~Logan, S.M.~Yang, B.A.~Grierson, R.~Nazikian and Q.~Yu, Nucl.\ Fusion {\bf 61}, 106006 (2021). 
...
\bibitem{rf4}  R.~Fitzpatrick, Phys.\ Plasmas {\bf 24}, 072506 (2017).
...
\bibitem{rf5} R.~Fitzpatrick,  Phys.\ Plasmas {\bf 32}, 062509 (2025).

\bibitem{ggj} A.H.~Glasser, J.M.~Greene and J.L.~Johnson, Phys.\ Fluids {\bf 19}, 567 (1976).

\bibitem{jet} L.~Frassinetti, S.~Saarelma, G.~Verdoolaege, M.~Groth, J.C.~Hillesheim, P.~Bilkova,
P.~Bohm, M.~Dunne, R.~Fridstr\"{o}m, E.~Giovannozzi, et al., Nucl.\ Fusion {\bf 61}, 016001 (2021).
...
\bibitem{diiid} R.~Nazikian, C.~Paz-Solden, J.D.~Callen, J.S.~deGrassie, D.~Eldon, T.E.~Evans, N.M.~Ferraro,
B.A.~Grierson, R.J.~Groebner, S.R.~Haskey, et al., Phys.\ Rev.\ Lett.\ {\bf 114}, 105002 (2015).
