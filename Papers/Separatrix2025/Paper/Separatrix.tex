...
systems (e.g., Hamada \cite{hamada}, Boozer \cite{bcoord}, PEST \cite{pest}, equal-arc), all of which have the required property that $q=q(\psi_p)$. For the sake of convenience, we have chosen to use PEST
coordinates, which are characterized by a toroidal angle, $\phi$, that corresponds to the polar angle in the $R$, $\phi$, $Z$ cylindrical 
coordinate system. (See Sect.~\ref{equil}.) Once the choice of toroidal angle is made, the coordinate system is uniquely defined. 
...
the straight-field-line coordinate system specified in Eq.~(\ref{fluxc}), and treating the region ${\mit\Psi}>{\mit\Psi}_c$ as a vacuum. Of course, this
is the approach taken in existing plasma response codes such as GPEC \cite{ipec,park,gpec,logan}, in which ${\mit\Psi}_c$ is given a value, lying just below unity, that is determined via an empirical guideline.  (This course of action is adopted because GPEC cannot deal with the singularity of the straight-field-line coordinate system at ${\mit\Psi}=1$, and
...
If we suppose that $q_{95}$ is gradually increased then, at some stage, a new rational surface will enter the domain of solution, $0<{\mit\Psi}<{\mit\Psi}_c$,
...
response to the RMP, because $|{\mit\Delta}_k|/|E_{kk}|$ is initially quite small, due to the large magnetic shear at ${\mit\Psi}={\mit\Psi}_c$. However, 
...
\bibitem{hamada} S.~Hamada,  Nucl.\ Fusion {\bf 2}, 23 (1962).

\bibitem{bcoord} A.H.~Boozer, Phys.\ Fluids {\bf 24}, 1999 (1981).

\bibitem{pest} J.L.~Johnson and R.C.~Grimm, Phys.\  Fluids {\bf 17}, 337 (1974).
