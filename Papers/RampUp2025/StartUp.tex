...
\begin{abstract}
A simple model of the ramp-up and ramp-down of the toroidal current in an ohmic tokamak plasma is developed. Faraday's law of electric induction
...
without encountering runaway electron problems during the current ramp suggests that the future SPARC and ITER tokamaks should certainly be able to
avoid such problems. 
\end{abstract}
\maketitle
...
7 seconds, and ramped down in 12 seconds, and seems to imply that the true ramp time is, in fact, 1157 seconds. The claim by de Vries et alia is
...
\frac{3}{2}\,n_e\,\frac{\partial T_e}{\partial t} - n_e\,\frac{1}{r}\,\frac{\partial}{\partial r}\left(r\,\chi_\perp\,\frac{\partial T_e}{\partial r}\right)
 ...
 \end{equation}
 whereas typical electric field-strength in the plasma takes the form
 \begin{equation}
...
In order to facilitate our analysis, we shall make two approximations. 
...
tokamak plasmas. (See also Table~\ref{t2}.) As is apparent from Eq.~(\ref{e17}), this is the case because the energy confinement time is much less than
...
invariably  possess a feedback system that automatically alters the instantaneous value of ${\cal E}(t)$ in such a manner as to give a prescribed plasma
...
\begin{equation}
\hat{I}_p(\hat{t}) =\hat{I}_p(\delta,\hat{t})= \frac{\lambda^3\,\delta}{(\delta\,{\cal E})^5}\,\bar{B}_\theta(1).
\end{equation}
...
\end{equation}
where $\hat{t}_{\rm ramp} =t_{\rm ramp}/\tau_R$. For the sake of simplicity, we shall assume that the plasma elongation is constant in time. 
...
is also satisfied at all times in the initial stage of the current ramp. Thus, we deduce that
\begin{equation}
...
\end{equation}
Thus, $\tau_{\rm min}$ represents the minimum possible safe current ramp-up timescale. 
...
same transformation in the relevant formulae given in Sect.~\ref{init}. In particular, the minimum safe current
...
Figure~\ref{fig4} shows a simulated current ramp for JET. Some of the characteristic  parameters for this ramp are listed in Table~\ref{t2}. 
...
Note that, as reported by de Vries et alia,\cite{run} the inductive electric field exceeds the Connor-Hastie field during the current ramp. However,
the electric field only exceeds de Vries et alia's empirical runaway electron generation threshold, which is ten times the Connor-Hastie field,\cite{run} in
the very early stages of the current ramp. Note that the threshold is exceeded, not because the current ramp is too fast, but rather because the
plasma is very cold and resistive in the early stages of the ramp. One obvious solution to runaway electron problems in the early
stages of a current ramp is to apply small amounts of electron cyclotron wave heating in order to slightly raise the electron temperature. 
The inductive electric field is slightly above the Connor-Hastie value at the end of the current ramp, but we would expect the field to
...
value throughout most of the current flattop. Note that the inductive electric field is much less than the Dreicer field throughout the current ramp, as is also
the case for our SPARC and ITER simulations. 

Figure~\ref{fig5} shows a simulated current ramp for SPARC. Some of the characteristic  parameters for this ramp are listed in Table~\ref{t2}. 
...
Note that the inductive electric field, ${\cal E}$,  exceeds the Connor-Hastie field, $E_c$,  during the current ramp. However, the ratio ${\cal E}/E_c$ for SPARC,
...
operating without dangerous levels of runaway electron generation during the current ramp, as reported by  Vries et alia,\cite{run} then SPARC
should certainly also be capable of operating without runaway electron problems. 

Finally, Fig.~\ref{fig6} shows a simulated current ramp for ITER. Some of the characteristic  parameters for this ramp are listed in Table~\ref{t2}. 
...
consistent with the series of equilibria shown in Fig.~1 of Ref.~\onlinecite{federici}, and is much less than the 15 MA in 60 s ramp-down rate envisioned by de Vries et alia.\cite{deVries}
Note that the inductive electric field, ${\cal E}$,  exceeds the Connor-Hastie field, $E_c$,  during the first third of the current ramp. However,  the ratio ${\cal E}/E_c$ for ITER, 
...
operating without dangerous levels of runaway electron generation during the current ramp, then ITER
should certainly also be capable operating without runaway electron problems.  

\section{Summary}
A simple model of the ramp-up and ramp-down of the toroidal current in an ohmic tokamak plasma is developed. Faraday's law of electric induction is found to 
...
This research was funded by the T U.S.\ Department of Energy, Office of Science, Office of Fusion Energy Sciences.
...
\bibitem{jet} Z.S.~Hartwig and Y.A.~Podpaly, {\em Magnetic Fusion Energy Formulary}, (MIT, Cambridge MA, 2011).
...
The JET parameters come from Ref.~\onlinecite{jet}. 
The SPARC and ITER parameters are taken from Tables 1 and 2 of Ref.~\onlinecite{creely}, as well as Table~1 of Ref.~\onlinecite{iter}. }\label{t1}
\end{table}
...
and the normalized electric field, ${\cal E}_{\rm ramp}$. The lower panel shows the ratio of the minimum safe current ramp
...
\caption{Equilibrium profiles calculated as functions of the normalized minor radius, $x$, for $\alpha=0$.  The top panel shows the normalized electron temperature profile, $\bar{T}$, and the
normalized current profile, $\bar{j}= \bar{T}^{\,3/2}$.  The bottom panel shows the safety-factor profile, $q(x)= q_a\,\bar{q}(x)$. }\label{fig3}
\end{figure}
