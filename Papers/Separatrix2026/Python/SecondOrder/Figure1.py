# Script to plot magnetic flux-surface in simple three-filament model
...
beta  = 0.2
delta = beta /2./(1.+beta)**0.5
qast  = 12.0

def Psi (X, Y):

    f1 = X*X + Y*Y
    f2 = (X + delta) * (X + delta) + (Y + 1.) * (Y + 1.)
    f3 = (X - delta) * (X - delta) + (Y + 1.) * (Y + 1.)

    return 0.5 * math.log (f1) + 0.25 * beta * math.log (f2) + 0.25 * beta * math.log (f3)
...
def PsiX (X, Y):
    ...
    return X /f1 + 0.5 * beta * (X + delta) /f2 + 0.5 * beta * (X - delta) /f3
...
def PsiY (X, Y):
    ...
    return Y /f1 + 0.5 * beta * (Y + 1.) /f2 + 0.5 * beta * (Y + 1.) /f3
...
X_x   = 0.
Y_x   = - (2. + beta) /2. /(1. + beta)
Psi_x = math.log (beta ** beta * (2. + beta) ** (1. + beta/2.) /2. ** (1. + beta) /(1. + beta) ** (1. + beta))

def F (Y):

    f1 = Y*Y
    f2 = delta*delta + (Y + 1.) * (Y + 1.)
...
def F90 (Y):
    ...
    return 0.5 * math.log (f1) + 0.5 * beta * math.log (f2) - 1.1*Psi_x


X     = 0.0
#Y     = root_scalar(F90, bracket=[0.01, 1.]).root
Y     = 0.94889*Y_c 
