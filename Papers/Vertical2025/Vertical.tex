...
The conventional way of increasing the total plasma current without degrading the  stability of the plasma to non-axisymmetric magnetohydrodynamical (MHD) modes is
to modify the plasma's poloidal cross-section by increasing its elongation and triangularity.\cite{jet} However, tokamak plasmas with strong cross-section shaping
are subject to severe axisymmetric instabilities.\cite{v1,v2} Such instabilities involve vertical motion of the plasma on the Alfv\'{e}n timescale, which results in the
sudden termination of the plasma discharge when it comes into contact with the first wall. It is possible to stabilize axisymmetric modes by placing a conducting wall
around the plasma. Usually, the vacuum vessel plays the role of this wall. The mode remains unstable if the wall possesses finite electrical conductivity,\cite{rwm} but its
growth time is increased from the Alfv\'{e}n time to the much longer L/R time of the wall.\cite{rwm1,rwm1a,rwm2} The L/R time is much shorter than the length of the plasma 
discharge, but is still long enough to allow active feedback stabilization of axisymmetric modes with practical power supplies.\cite{rwm3}

Ref.~\onlinecite{tj} describes the TJ toroidal tearing mode code. This code calculates the stability of an inverse-aspect ratio expanded  tokamak plasma equilibrium  to {\em non-axisymmetric}\/ tearing modes via
asymptotic matching techniques. Ref.~\onlinecite{tj1} describes a generalization of the TJ code that allows it to calculate the stability of the plasma to non-axisymmetric
ideal modes in the presence of a perfectly conducting wall that surrounds the plasma. The aim of this paper is to further generalize the TJ code to allow it to calculate
...
$n^{-1}$ in the analysis of Ref.~\onlinecite{tj}. Hence, as described in this paper, it is necessary to redo the much of the analysis for the special case $n=0$. 
...
Let $\mu$, $\eta$, $\phi$ be right-handed toroidal coordinates defined such that\,\cite{tj,tj1,mf}
\begin{align}
...
where
$\nabla^2 V =0$.
The most general axisymmetric  solution to Laplace's equation is\,\cite{mf1}
\begin{align}
...
According to Eqs.~(\ref{e41}) and (\ref{e54})
...
Suppose that the plasma is surrounded by a vacuum region that is bounded by a perfectly conducting wall that lies at $r=b_w\,a$, where $b_w\geq 1$. 
Because the wall is perfectly conducting, 
$\underline{\psi}(r=b_w\,a)=0$.
In other words, the normal component of the perturbed magnetic field is zero at the wall. 
...
\begin{equation}
 ...
 is termed the {\em perfect-wall vacuum matrix}.
  Here, $\underline{\underline{\cal Q}}_{\,a}= \underline{\underline{\cal Q}}(r=a_+)$, et cetera. 
...
contributions to the $\lambda_m$, respectively. 
 The eigenfunction of the $m$th mode is conveniently normalized such that $y_{m'}(a)= \beta_{mm'}$. 
...
Consider a particular ideal mode for which the total perturbed potential energy in the absence of a wall, 
 $\delta W_{nw}$, is negative,  but the total perturbed potential energy in the presence of a perfectly conducting wall, $\delta W_{pw}$, is positive.  In this case,  the no-wall ideal mode is unstable (because $\delta W_{nw}<0$). In the absence of a the wall, this mode would grow on the Alfv\'{e}n time. On the other
