...
Positive triangularity is found to have a
strong stabilizing effect on the vertical mode in vertically elongated plasmas, whereas
negative triangularity is either destabilizing or weakly stabilizing. It is concluded that finite wall thickness should be taken into account when
...
$n^{-1}$ in the analysis of Ref.~\onlinecite{tj}. Hence, as described in Sect.~\ref{deriv}, it is necessary to redo  much of the TJ analysis for the special case $n=0$. 
...
is
$(\nabla R\times \nabla\phi\cdot\nabla Z)^{-1} = R$. 
Note that $|\nabla\phi|=1/R$. The cylindrical axis is assumed to coincide with the symmetry axis of the toroidal plasma equilibrium, which
is assumed to have perfectly-nested toroidal magnetic flux-surfaces. 

We can write the divergence-free equilibrium magnetic field in the usual Clebsch form\,\cite{boozer}
\begin{equation}
...
and $\alpha_f(r)$ are defined in Eqs.~(\ref{ag}), (\ref{alpp}), and (\ref{alpf}), respectively. Note that the axisymmetric analysis  of Sect.~\ref{deriv}
...
In principle, there is a contribution to the perturbed potential energy from the plasma-vacuum interface. This
...
In order to determine the growth-rate of the axisymmetric resistive wall mode, we need to calculate the total perturbed 
...
\end{align}
Note that Eq.~(\ref{ety}) only applies to $m\neq 0$.
...
\begin{equation}
\frac{\partial e_\phi}{\partial\eta} = - {\rm i}\,\gamma\,h\,\sinh\mu\,\frac{\partial V}{\partial \mu}= - {\rm i}\,\gamma\,h\,(z^2-1)\,\frac{\partial V}{\partial z}. 
\end{equation}
Thus,
\begin{align}
{\cal E}(z) &= {\rm i}\,\frac{\pi}{2}\oint\left(\frac{\partial e_\phi}{\partial\eta}\,V^\ast - \frac{\partial e_\phi^{\,\ast}}{\partial\eta}\,V\right)d\eta= 
\frac{\pi\,\gamma}{2}\oint h\,(z^2-1)\left(\frac{\partial V}{\partial z}\,V^\ast - \frac{\partial V^\ast}{\partial z}\,V\right) d\eta\nonumber\\[0.5ex]
&= -\pi^2\,\gamma\sum_m(p_m\,q_m^{\,\ast}-q_m\,p_m^{\,\ast})\,(z^2-1)\,{\cal W}(P_{|m|-1/2},Q_{|m|-1/2}),
 \end{align}
 ...
 {\cal E}(z)=\,\pi^2\,\gamma\sum_m(p_m\,q_m^{\,\ast}-q_m\,p_m^{\,\ast}).
 \end{equation}
...
\begin{equation}
{\cal E}(r) = -\pi^2\,\gamma\, (\underline{V}^\dag\,\underline{\psi}- \underline{\psi}^\dag\,\underline{V}).
\end{equation}
...
{\cal E}(r) =\pi^2\,\gamma\,(\underline{q}^\dag\,\underline{p}-\underline{p}^\dag\,\underline{q}).
\end{equation}
...
\end{align}
The previous five equations hold throughout the vacuum region. In fact, checking that these relations hold in the vacuum region constitutes a good numerical
...
Consider a particular axisymmetric ideal mode for which the total perturbed potential energy in the absence of a wall, 
 ...
 \end{equation}
 is the (unnormalized) L/R time of the wall. (See Sect.~\ref{loverr}.) 
...
(The relationship between these two angles is specified in Sect.~\ref{trans}.)
...
\end{align}
Here, $p_2$, ${\mit\Sigma}$, and $S_4$ are defined in Eqs.~(\ref{eq1}), and (\ref{el1})--(\ref{el5}). 
...
\end{equation}
Plasmas whose current profiles exhibit strong central peaking tend to have high $l_i$ values, whereas those with
...
\begin{align}
q_{\rm cly}(r) &= \frac{q(0)\,\nu_q\,\hat{r}^{\,2}}{1-(1-\hat{r}^{\,2})^{\nu_q}},\\[0.5ex]
p_2(r)&=p_2(0)\,(1-\hat{r}^{\,2})^{\nu_p}
\end{align}
for $\hat{r}\leq 1$, and
\begin{align}
q_{\rm cly}(r) &= q(0)\,\nu_q\,\hat{r}^{\,2}\\[0.5ex]
...
Note that $q(r)$, $s(r)\equiv d\ln q/d\ln\hat{r}$, and $p_2'(r)$ are all continuous across the plasma-vacuum interface, as is assumed to be the case in our
...
In summary, our model equilibria are controlled by eight parameters: $q(0)$, $q(a)$, $p_2(0)$, $\nu_p$, $H_2(a)$, and $H_3(a)$, as well as the inverse aspect-ratio, $\epsilon$,
...
plasma elongation parameter, $H_2(a)$, for plasma equilibria characterized by  $\epsilon=0.1$, $H_3(a)=0$, $q(0)=1.01$, $q(a)=3.6$,  $p_2(0)=0.05$, $\nu_p=2.5$, and $b_w=1.1$. \label{fig1}}
\end{figure}
...
\caption{The no-wall perturbed potential energy, $\delta W_{nw}$, the perfect-wall perturbed potential energy, $\delta W_{pw}$, the wall
parameter, $\alpha_w$, and the normalized thin-wall growth-rate, $\hat{\gamma}$,  of the  vertical mode, plotted as  functions of the
plasma  inverse aspect-ratio, $\epsilon$, for plasma equilibria characterized by   $H_3(a)=0$, $q(0)=1.01$, $q(a)=3.6$,  $p_2(0)=0.05$, $\nu_p=2.5$, $b_w=1.1$, and
...
\caption{The no-wall perturbed potential energy, $\delta W_{nw}$, the perfect-wall perturbed potential energy, $\delta W_{pw}$, the wall
parameter, $\alpha_w$, and the normalized thin-wall growth-rate, $\hat{\gamma}$,  of the  vertical  mode, plotted as  functions of the
plasma triangularity parameter, $H_3(a)$, for plasma equilibria characterized by  $\epsilon=0.1$, $H_2(a)=1$, $q(0)=1.01$, $q(a)=3.6$,  $\nu_p=2.5$, $b_w=1.1$, and
...
\caption{The no-wall perturbed potential energy, $\delta W_{nw}$, the perfect-wall perturbed potential energy, $\delta W_{pw}$, the wall
parameter, $\alpha_w$, and the normalized thin-wall growth-rate, $\hat{\gamma}$,  of the vertical mode, plotted as  functions of the
relative wall radius, $b_w$, for plasma equilibria characterized by  $\epsilon=0.1$, $H_2(a)=1$, $H_3(a)=0$, $q(0)=1.01$, $q(a)=3.6$, $\nu_p=2.5$, and various different values of $p_2(0)$.\label{fig6}}
\end{figure}
...
fixed $q(a)$. It can be seen that  non-triangular plasmas exhibit an increase of the growth-rate of the vertical mode with increasing $l_i$, indicating that the vertical
...
\caption{The no-wall perturbed potential energy, $\delta W_{nw}$, the perfect-wall perturbed potential energy, $\delta W_{pw}$, the wall
parameter, $\alpha_w$, and the normalized thin-wall growth-rate, $\hat{\gamma}$,  of the vertical mode, plotted as  functions of the
plasma self-inductance, $l_i$, for plasma equilibria characterized by  $\epsilon=0.1$, $H_2(a)=1$,   $q(a)=3.6$, $p_2=0.05$, $\nu_p=2.5$, $b_w=1.1$,  and various different values of $H_3(a)$.\label{fig7}}
\end{figure}
...
The calculation described in this paper has been implemented in the TJ toroidal tearing mode code. (This is possible because the basis solutions used to construct
tearing eigenfunctions in TJ can  be repurposed to construct ideal eigenfunctions.) 
The results obtained from the enhanced TJ code are broadly similar to those reported previously in the literature.\cite{f2,hump,reb,ros,ward,guz} In particular, positive triangularity is
found to have a strong stabilizing effect on the vertical mode, whereas negative triangularity either destabilizing or weakly stabilizing. Finally, our calculations indicate that finite wall thickness should be taken into account when
...
\bibitem{hump} D.A.~Humphreys, N.W.~Eidietis, J.R.~Ferron, G.L.~Jackson, M.J.~Lanctot, M.L.~Walker, 
...
\bibitem{reb} E.~Rebhan and A.~Salat, Nucl.\ Fusion {\bf 16}, 805 (1976).

\bibitem{ros} M.D.~Rosen, Phys.\ Fluids {\bf 18}, 482 (1975). 
...
\bibitem{ward}  D.J.~Ward,  A.~Bondeson and F.~Hoffmann, Nucl.\ Fusion {\bf 33}, 821 (1993). 
...
\end{equation}
in accordance with Eq.~(\ref{jac}). Note, from Eqs.~(\ref{e12}) and (\ref{e13}) that
\begin{equation}
...
\end{align}
The horizontal {\em Shafranov shift}\/ of equilibrium magnetic flux-surfaces is parameterized by $-H_1$.
