...
An additional constraint that applies to tokamak reactors, but not stellarator reactors,  is that the inductive electric field that drives the plasma current must be kept sufficiently
...
$q_a$ is held constant. (See Fig.~7 of Ref.~\cite{lister}.) Finally, $q_a$ is allowed to rise to a large value, at fixed minor radius,  as last remnants of the plasma current disappear. 
...
\end{tabular}
\caption{Plasma parameters at end of the current ramp-up for the JET, SPARC, and ITER tokamaks. Here, $I_p$ is the toroidal plasma current, $P$ the
