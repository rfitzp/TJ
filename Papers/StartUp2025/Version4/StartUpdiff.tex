...
%DIF DEL StartUpOld.tex   Thu Sep 25 11:31:50 2025
%DIF ADD StartUp.tex      Thu Sep 25 11:30:06 2025
\usepackage{amsmath}           		          	
...
ITER}\DIFdelend \DIFaddbegin \DIFadd{ITER, and DEMO }\DIFaddend tokamaks are 4.2, 2.0, \DIFdelbegin \DIFdel{and }\DIFdelend 14.7\DIFaddbegin \DIFadd{, and 38.4 }\DIFaddend seconds, respectively. \DIFaddbegin \DIFadd{These estimates are obtained for ohmic ramp phases with relatively low
flat-top temperatures. 
}\DIFaddend The JET ramp time is in accordance with operational experience. The SPARC\DIFdelbegin \DIFdel{and ITER}\DIFdelend \DIFaddbegin \DIFadd{, ITER, and DEMO }\DIFaddend minimum safe ramp times are less than the ramp times in the respective designs. Hence, there is no indication that the design ramp times are infeasible, as was recently 
suggested  in arXiv:2507.05456\DIFaddbegin \DIFadd{v1 }\DIFaddend (2025).
The typical ratios of the inductive electric field
to the Connor-Hastie field in SPARC\DIFdelbegin \DIFdel{and ITER}\DIFdelend \DIFaddbegin \DIFadd{, ITER, and DEMO during current ramps }\DIFaddend are found to be less than those in JET. Thus, the fact that the JET tokamak was able to operate successfully
without encountering runaway electron problems during current ramps suggests that the future SPARC\DIFdelbegin \DIFdel{and ITER}\DIFdelend \DIFaddbegin \DIFadd{, ITER, and DEMO }\DIFaddend tokamaks should also be able to
avoid such problems. 
\end{abstract}
...
An additional constraint that applies to tokamak reactors, but not \DIFaddbegin \DIFadd{to }\DIFaddend stellarator reactors,  is that the inductive electric field that drives the plasma current must be kept sufficiently
...
In a recent paper, Boozer\,\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{boozer} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{boozer,boozera} }\hskip0pt%DIFAUXCMD
...
actually do place on future tokamak operation. Boozer\,\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{boozer,boozer1}  }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{boozer,boozera}  }\hskip0pt%DIFAUXCMD
...
$q_a$ is held constant. (See Fig.~7 of Ref.~\cite{lister}.) Finally, $q_a$ is allowed to rise to a large value, at fixed minor radius,  as \DIFaddbegin \DIFadd{the }\DIFaddend last remnants of the plasma current disappear. 

\DIFaddbegin \DIFadd{It should be noted that our treatment of the ramp-down phase, in particular, is a considerable oversimplification. In reality,
...
plasma facing components, and to also mitigate the influx of tungsten impurities \mbox{%DIFAUXCMD
\cite{poli}}\hskip0pt%DIFAUXCMD
. Moreover, the ITER ramp-down will be initiated with
dominant alpha heating \mbox{%DIFAUXCMD
\cite{deVries}}\hskip0pt%DIFAUXCMD
. Finally, the presence of strongly radiating tungsten impurities in the ITER plasma core during the ramp-down may require auxiliary
electron heating to avoid the occurrence of  highly destabilizing hollow temperature and current profiles \mbox{%DIFAUXCMD
\cite{w1,w2}}\hskip0pt%DIFAUXCMD
...
\DIFadd{We are implicitly assuming that the plasma enters and exits H-mode during the current flat-top phase,  when the auxiliary heating is turned on and off, respectively.
...
that can lead to major disruptions \cite{wesson,cheng}. \DIFaddbegin \DIFadd{Peaked current profiles also make it difficult to maintain the vertical stability of the plasma \mbox{%DIFAUXCMD
\cite{mul}}\hskip0pt%DIFAUXCMD
. }\DIFaddend Thus, the criterion (\ref{e35}) is a necessary one for  the completely safe operation of a tokamak during the
...
\begin{equation}\DIFaddbegin \label{e46c}
\DIFaddend \hat{T}(\hat{r},\hat{t}) = \frac{\lambda^2}{(\delta\,{\cal E})^4}\,\bar{T}(x)\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
\DIFaddbegin \DIFadd{where $\lambda$ is a dimensionless constant. 
}\DIFaddend Equation~(\ref{e38}) transforms to give
...
\begin{align}\DIFaddbegin \label{e48c}
\DIFaddend \bar{T}(0) &=1,\\[0.5ex]
...
Let 
\begin{equation}\DIFaddbegin \label{e51c}
\DIFaddend \hat{B_\theta}(\hat{r},\hat{t}) = \frac{\lambda^3}{(\delta\,{\cal E})^5}\,\bar{B}_\theta(x).
\end{equation}
Equation~(\ref{e39}) yields 
\begin{equation}\DIFaddbegin \label{e52c}
\DIFaddend \frac{d\bar{B}_\theta}{dx} + \frac{\bar{B}_\theta}{x} = \bar{T}^{\,3/2},
\end{equation}
which must be solved subject to the boundary condition
\begin{equation}\DIFaddbegin \label{e53c}
\DIFaddend \frac{d\bar{B}(0)}{dx}=0.
\end{equation}

\DIFdelbegin \DIFdel{Let
}\DIFdelend \DIFaddbegin \DIFadd{Note that the rescalings (\ref{e46c}) and (\ref{e51c}) are the only possible ones that leave Eqs.~(\ref{e41}) and (\ref{e52c}) independent of the time-dependent quantities 
...
\DIFadd{Let
}\DIFaddend \begin{equation}\DIFaddbegin \label{e54c}
\DIFaddend q(\hat{r},\hat{t}) =\frac{q_a\,\delta\,(\delta\,{\cal E})^5}{\lambda^3}\,\bar{q}(x).
\end{equation}
Equation~(\ref{e28}) gives
\begin{equation}\DIFaddbegin \label{e55c}
\DIFaddend \bar{q}(x) = \frac{x}{\bar{B}_\theta(x)}.
\end{equation}
Note that
\begin{equation}\DIFaddbegin \label{e56c}
\DIFaddend \bar{q}(0)= 2.
\end{equation}
According to Eq.~(\ref{e29}), the net normalized toroidal current flowing in the plasma is
\begin{equation}\DIFaddbegin \label{e57c}
\DIFaddend \hat{I}_p(\hat{t}) \equiv \hat{I}_p(\delta,\hat{t})= \frac{\lambda^3\,\delta}{(\delta\,{\cal E})^5}\,\bar{B}_\theta(1).
\end{equation}
Equation~(\ref{e36}) implies that the normalized ohmic heating power is
\begin{equation}\DIFaddbegin \label{e58c}
\DIFaddend \hat{P}(\hat{t})= \frac{\lambda^2}{(\delta\,{\cal E})^4}\,\zeta,
\end{equation}
where
\begin{equation}
...
boundary is
\begin{equation}\DIFaddbegin \label{e60c}
\DIFaddend \hat{\psi}_{\rm tot}(\hat{t}) = \frac{\lambda^{\,3}\,\delta}{(\delta\,{\cal E})^5}\,\int_0^1\bar{B}_\theta\,dx.
\end{equation}
...
sawtooth oscillation effectively prevents the central safety-factor from falling significantly below unity \cite{book}. It follows \DIFaddbegin \DIFadd{from Eqs.~(\ref{e55c}),
(\ref{e56c}), and (\ref{e57}) }\DIFaddend that
\begin{equation}
q_a = \frac{1}{2\,\bar{B}_\theta(1)}.
\end{equation}
Equations~(\ref{e56}) and (\ref{e57}) allow us to determine all other quantities in the initial
stage of the ramp-up. \DIFdelbegin \DIFdel{We obtain
}\DIFdelend \DIFaddbegin \DIFadd{Thus, comparing the value of $\hat{I}_p(\hat{t})\,q(1,\hat{t})$ obtained from Eqs.~(\ref{e54c}), (\ref{e55c}), and (\ref{e57c})
...
\end{equation}
which implies that the plasma minor radius takes a relatively small constant value during the first stage of the ramp-up. \DIFdelbegin \DIFdel{Furthermore, }\DIFdelend \DIFaddbegin \DIFadd{A comparison of
...
P_{\rm ramp} &= \zeta\,T_{\rm ramp}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{align}
\DIFaddbegin \DIFadd{and use has been made of Eqs.~(\ref{e26c}), (\ref{e46c}), (\ref{e51c}), (\ref{e58c}), and (\ref{e60c}). 
}\DIFaddend Finally, the criterion (\ref{e55}) yields
...
stage of the ramp-up. \DIFdelbegin \DIFdel{We find that 
}\begin{align*}
\DIFdel{\delta }&\DIFdel{= \left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)^{1/2},%DIFDELCMD < \label{e70}%%%
}\\[0.5ex]
\DIFdel{\hat{T}(x,\hat{t}) }&\DIFdel{= T_{\rm ramp}\left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)^{2/5}\bar{T}(x),}\\[0.5ex]
\DIFdel{\hat{B}_\theta(x,\hat{t})}&\DIFdel{= \left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)^{1/2}\frac{\bar{B}_\theta(x)}{\bar{B}_\theta(1)},}\\[0.5ex]
{\DIFdel{\cal E}}\DIFdel{(\hat{t}) }&\DIFdel{= }{\DIFdel{\cal E}}\DIFdel{_{\rm ramp}\left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)^{-3/5},}\\[0.5ex]
\DIFdel{\hat{P}(\hat{t}) }&\DIFdel{= P_{\rm ramp}\left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)^{2/5},}\\[0.5ex]
\DIFdel{\hat{\psi}_{\rm tot}(\hat{t})}&\DIFdel{= \left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)\int_0^1\frac{\bar{B}_{\theta}(x)}{\bar{B}_\theta(1)}\,dx,}\\[0.5ex]
\DIFdel{\bar{n}(\hat{t}) }& \DIFdel{= \bar{n}_0.
}\end{align*}%DIFAUXCMD
\DIFdel{It is clear from Eq}\DIFdelend \DIFaddbegin \DIFadd{Thus, comparing the value of $\hat{I}_p(\hat{t})\,q(1,\hat{t})$ obtained from Eqs}\DIFaddend .~(\DIFdelbegin \DIFdel{\ref{e70})that}\DIFdelend \DIFaddbegin \DIFadd{\ref{e54c}), (\ref{e55c}), and (\ref{e57c})
with the value obtained from Eqs.~(\ref{e56}) and (\ref{e57a}), we deduce that 
}\begin{equation}
\DIFadd{\delta = \left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)^{1/2},\label{e70}
}\end{equation}
\DIFadd{which implies that}\DIFaddend , in order to keep the edge safety-factor constant, while the plasma current increases in time, it
is necessary for the minor radius of the plasma to also increase in time. \DIFaddbegin \DIFadd{A comparison of
Eqs.~(\ref{e57c}) and (\ref{e56}) then gives  ${\cal E}^5 = \lambda^3\,\bar{B}_\theta(1)\,\,(\hat{t}/\hat{t}_{\rm ramp})^{-3}$. 
Thus, we obtain
}\begin{align}
\DIFadd{\hat{T}(x,\hat{t}) }&\DIFadd{= T_{\rm ramp}\left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)^{2/5}\bar{T}(x),}\\[0.5ex]
\DIFadd{\hat{B}_\theta(x,\hat{t})}&\DIFadd{= \left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)^{1/2}\frac{\bar{B}_\theta(x)}{\bar{B}_\theta(1)},}\\[0.5ex]
{\DIFadd{\cal E}}\DIFadd{(\hat{t}) }&\DIFadd{= }{\DIFadd{\cal E}}\DIFadd{_{\rm ramp}\left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)^{-3/5},}\\[0.5ex]
\DIFadd{\hat{P}(\hat{t}) }&\DIFadd{= P_{\rm ramp}\left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)^{2/5},}\\[0.5ex]
\DIFadd{\hat{\psi}_{\rm tot}(\hat{t})}&\DIFadd{= \left(\frac{\hat{t}}{\hat{t}_{\rm ramp}}\right)\int_0^1\frac{\bar{B}_{\theta}(x)}{\bar{B}_\theta(1)}\,dx,}\\[0.5ex]
\DIFadd{\bar{n}(\hat{t}) }& \DIFadd{= \bar{n}_0.
}\end{align}
\DIFaddend 
...
In this section, we shall describe simulated current ramp-ups made using our model for the JET, SPARC,  \DIFdelbegin \DIFdel{and ITER}\DIFdelend \DIFaddbegin \DIFadd{ITER, and DEMO  }\DIFaddend tokamaks. The
machine parameters for these tokamaks are given in Table~\ref{t1}. The simulations are made assuming that \DIFdelbegin \DIFdel{$\chi_0=1\,{\rm m\,s}^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{$\chi_0=1\,{\rm m^2\,s}^{-1}$}\DIFaddend ,
$\ln{\mit\Lambda} = 15$, $Z=2$, $t_{\rm pre}= 0.05\,t_{\rm ramp}$, and $t_{\rm ramp} =\tau_{\rm min}$. 
Note that\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{, in all devices,  the choice $\chi_0=1\,{\rm m^2\,s}^{-1}$ is found to be reasonably consistent with the ITER98(y,2) scaling law \mbox{%DIFAUXCMD
\cite{scaling}}\hskip0pt%DIFAUXCMD
, assuming that the
heating power is ohmic. 
Note, further, that the }\DIFaddend current ramp time in these simulations takes its
...
\DIFdelbegin \DIFdel{Finally, Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig6} shows a simulated current ramp-up for ITER. Some of the characteristic  parameters for this ramp-up are listed in Table~\ref{t2}. 
...
\DIFaddbegin \DIFadd{Finally, Fig.~\ref{fig7} shows a simulated current ramp-up for DEMO. Some of the characteristic  parameters for this ramp-up are listed in Table~\ref{t2}. 
It can be seen that the toroidal plasma current in DEMO can be safely ramped up to about 20 MA in about 38 seconds. This time is less than the ramp-down times estimates in the much more detailed study contained in Ref.~\mbox{%DIFAUXCMD
\cite{mul}}\hskip0pt%DIFAUXCMD
. Note that the electron number density is above the Greenwald
density during the main part of the ramp-up, because DEMO plans to operate at densities in excess of the Greenwald density. (See Table~\ref{t1} and Ref.~\mbox{%DIFAUXCMD
\cite{demo}}\hskip0pt%DIFAUXCMD
.) The ratio ${\cal E}/E_c$
...
\DIFdelbegin \DIFdel{and
ITER}\DIFdelend \DIFaddbegin \DIFadd{ITER, and DEMO }\DIFaddend tokamaks are 4.2, 2.0, \DIFdelbegin \DIFdel{and }\DIFdelend 14.7\DIFaddbegin \DIFadd{, and 38.4 }\DIFaddend seconds, respectively. \DIFaddbegin \DIFadd{These estimates are obtained for ohmic ramp phases with relatively low
flat-top temperatures. }\DIFaddend The JET ramp time is in accordance with operational experience. The SPARC\DIFdelbegin \DIFdel{and ITER}\DIFdelend \DIFaddbegin \DIFadd{, ITER, and DEMO }\DIFaddend minimum safe ramp times are less than the ramp times in the respective designs. Hence, there is no indication that the design ramp times are infeasible, as was recently 
suggested by Boozer \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{boozer}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{boozer,boozera}}\hskip0pt%DIFAUXCMD
}\DIFaddend . The reason that the simple calculation of the resistive evolution timescale, $\tau_R$, gives values that are greatly in
...
to the Connor-Hastie field in SPARC\DIFdelbegin \DIFdel{and ITER}\DIFdelend \DIFaddbegin \DIFadd{, ITER, and DEMO during current ramps }\DIFaddend are less than those in JET. Thus, the fact that the JET tokamak was able to operate successfully
without encountering runaway electron problems during  current ramps suggests that the future SPARC\DIFdelbegin \DIFdel{and ITER}\DIFdelend \DIFaddbegin \DIFadd{, ITER, and DEMO }\DIFaddend tokamaks should certainly be able to
avoid such problems. 

\DIFdelbegin \DIFdel{It should be understood that }\DIFdelend \DIFaddbegin \DIFadd{As has already been discussed in Sect.~\ref{operation}, }\DIFaddend the model presented in this paper is very much an oversimplification. In reality, other issues than the minimum time at which
...
\DIFaddbegin \DIFadd{Some of these issues are discussed in Refs.~\mbox{%DIFAUXCMD
\cite{mul} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{kok}}\hskip0pt%DIFAUXCMD
. 
}\DIFaddend In the case of current ramp-up, the plasma generally starts off with a comparatively low electron number density (because there is a practical upper limit
...
is initially pushed downward and crushed against the magnetic X-point (this is common practice in modern \DIFdelbegin \DIFdel{tokamaks}\DIFdelend \DIFaddbegin \DIFadd{tokamaks---see Fig.~2 in Ref.~\mbox{%DIFAUXCMD
\cite{mul}}\hskip0pt%DIFAUXCMD
}\DIFaddend ). The main issue
...
\funding{This research was funded by the  U.S.\ Department of Energy, Office of Science, Office of Fusion Energy Sciences under contract DE-SC0021156.}
\ack{The author gratefully acknowledges informative discussions with A.H.~Boozer, R.~Granetz, A.J.~Creely, M.~Greenwald, B.~Breizman, and P.C.~de\,Vries.} 

\data{The digital data used in the figures in this paper can be obtained from the author upon reasonable request.}

\begin{thebibliography}{99}\baselineskip 5ex

\bibitem{boozer}   A.H.~Boozer, arXiv preprint arXiv:2507.05456\DIFaddbegin \DIFadd{v1 }\DIFaddend (2025). 

\DIFaddbegin \bibitem{boozera}  \DIFadd{A.H.~Boozer, private communication (2025).
}

\DIFaddend \bibitem{deVries} P.C.~de Vries, T.C.~Luce, Y.S.~Bae, S.~Gerhardt, X.~Gong, Y.~Gribov, 
...
T.~F\"{u}lop, D.T.~Garnier, R.~Granetz, \DIFaddbegin \DIFadd{T.K.~Gray, }\DIFaddend et al.,  J.\ Plasma Phys.\ {\bf 86}, 865860502 (2020).
...
\DIFaddbegin \bibitem{poli} \DIFadd{F.M.~Poli, S.-H.~Kim, P.C.~de Vries, Y.~Gribov, A.~Polevoi, F.~Koechl, A.~Kalvin, R.~Khayrutdinov, V.~Lukash
and ITPA-IOS members and experts, }{\em \DIFadd{The Plasma Current Ramp-Down in ITER: Physics Constraints on Control}}\DIFadd{, 
Proc.\   27th  IAEA Fusion Energy Conf., 2018. 
  }

\bibitem{w1} \DIFadd{D.~Fajardo, C.~Angioni, S.H.~Kim, F.~Koechl, E.~Fable, A.~Loarte, A.~Polevoi, G.~Tardini and the ASDEX Upgrade Team,
Plasma Phys.\ Control.\ Fusion }{\bf \DIFadd{67}}\DIFadd{, 015020 (2025). 
}

\bibitem{w2} \DIFadd{C.~Sozzi, E.~Alessi, P.J.~Lomas, F.~Rimini, C.~Stuart, C.~Challis, L.~Garzotti, M.~Lennhold, S.~Gerasimov, 
C.~Maggi, et al., }{\em \DIFadd{Termination of Discharges in High Performance Scenarios in JET}}\DIFadd{, Proc.\  28th IAEA Fusion Energy Conf., 2021. 
}

\DIFaddend \bibitem{uckam} N.A.~Uckan and the ITER Physics Group, {\em \DIFaddbegin \DIFadd{International Atomic Energy Agency, }\DIFaddend ITER Physics Design Guidelines: 1989},  (IAEA, Vienna, 1990).
...
\DIFaddbegin \bibitem{mul} \DIFadd{S.~Van Mulders, O.~Sauter, C.~Contr\'{e}, E.~Fable, F.~Felici, P.~Manas, M.~Mattei, F.~Palermo, M.~Siccinio and A.A.~Teplukhina,
Plasma Phys.\ Control.\ Fusion }{\bf \DIFadd{66}}\DIFadd{, 025007 (2024).
}

\bibitem{scaling} \DIFadd{ITER Physics Expert Group on Confinement and Transport, et al., Nucl.\ Fusion
}{\bf \DIFadd{39}}\DIFadd{,  2175 (1999). 
}

\DIFaddend \bibitem{jet} I.~Voitsekhovitch, A.C.C.~Sips, B.~Alper, M.~Beurskens, I.~Coffey,
J.~Conboy, T.~Gerbaud, C.~\DIFdelbegin \DIFdel{Giroud1}\DIFdelend \DIFaddbegin \DIFadd{Giroud}\DIFaddend , T.~Johnson, F.~K\"{o}ch, et al., Plasma Phys.\ Control. Fusion {\bf 52}  105011 (2010). 
...
\DIFaddbegin \bibitem{demo} \DIFadd{M.~Siccinio, J.P.~Graves, R.~Kembelton, H.~Lux, F.~Maviglia, A.W.~Morris, J.~Morris and H.~Zohm, Fus.\ Eng.\ Design }{\bf \DIFadd{176}}\DIFadd{, 113047 (2022). 
}

\bibitem{kok} \DIFadd{F.~Koechl, R.~Ambrosino, P.~Belo, M.~Cavinato, G.~Corrigan, L.~Garzotti, D.~Harting, A.~Kukushkin, A.~Loarte, M.~Mattei,
et al.\ Nucl.\ Fusion }{\bf \DIFadd{60}}\DIFadd{, 066015 (2020).
}

\DIFaddend \bibitem{deVries1} P.C.~de Vries, private communication (2025).
...
ITER & 6.2 & 2.0 & 5.3 & 1.0 &1.12& 1.85\DIFaddbeginFL \\[0.5ex]
\DIFaddFL{DEMO }&\DIFaddFL{9.07 }& \DIFaddFL{2.93 }& \DIFaddFL{5.86}& \DIFaddFL{0.88 }& \DIFaddFL{0.735}& \DIFaddFL{1.65
}\DIFaddendFL \end{tabular}
\caption{Machine parameters for the JET, SPARC,  \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL ITER\DIFaddbeginFL \DIFaddFL{, and EU-DEMO(2018) }\DIFaddendFL tokamaks. 
...
\DIFaddbeginFL \DIFaddFL{The DEMO parameters are taken from Table 1 of Ref.~\mbox{%DIFAUXCMD
\cite{demo}}\hskip0pt%DIFAUXCMD
. 
}\DIFaddendFL Here, $R_0$ is the major radius, $a$ the minor radius, $B_0$ the toroidal magnetic field-strength, $n_e$ the electron number
...
ITER & 14.0 & 14.4 & 1.61  &0.0583& 0.0332&0.0765& 50.4&14.7\DIFaddbeginFL \\[0.5ex]
\DIFaddFL{DEMO }&\DIFaddFL{19.8 }& \DIFaddFL{20.2 }& \DIFaddFL{1.84 }& \DIFaddFL{0.0477 }& \DIFaddFL{0.0206 }& \DIFaddFL{0.0673 }& \DIFaddFL{38.9}& \DIFaddFL{38.4
}\DIFaddendFL \end{tabular}
\caption{Plasma parameters at end of the current ramp-up for the JET, SPARC, \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL ITER\DIFaddbeginFL \DIFaddFL{, and DEMO }\DIFaddendFL tokamaks. Here, $I_p$ is the toroidal plasma current, $P$ the
...
\caption{\DIFaddFL{Simulated current ramp-up for the DEMO tokamak. See caption to Fig.~\ref{fig4}.}}\label{fig7}
