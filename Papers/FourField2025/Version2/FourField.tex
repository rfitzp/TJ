...
\title{\bf A four-field resonant response model for tokamak plasmas with applications to pedestal physics}
...
are intrinsically stable to tearing perturbations \cite{sco,compass,fish,rf1993,rf1998}. RMPs arise naturally as a consequence  of magnetic
field-coil misalignments  \cite{error}, but are sometimes deliberately applied to tokamak plasmas in order to suppress edge localized modes (ELMs) \cite{elm}. 
...
Obviously, this neglect is not going to make a difference if the ion sound parameter, $c_\beta$, is much less than unity, which explains
...
the neglect of the $c_\beta^{\,2}$ term has a larger relative effect on the real part of the scaled layer response index than on the imaginary part.
...
that occurs, in some response regimes,  close to the MHD-fluid resonance ($Q_E=0$). All four figures indicate that if the $c_\beta^{\,2}$ term is neglected then the imaginary part
of the response index passes through zero exactly at the electron fluid resonance ($Q_E+Q_e=0$). This implies that the electromagnetic
torque exerted by the RMP on the plasma in the resonant layer is zero at the electron fluid resonance. However, as is
clear from the figures, when the $c_\beta^{\,2}$ term is retained then the $Q_E$ value at which ${\rm Im}(\hat{\mit\Delta})=0$ is shifted from the
electron fluid resonance in the ion diamagnetic direction [i.e., ${\rm Im}(\hat{\mit\Delta})=0$ when $Q_E+Q_e<0$] \cite{lee}. 

Figure~\ref{fig5} shows the difference between the scaled layer response index predicted by the three-field Fourier-space  resonant response model of Ref.~\cite{rf2022} and the
...
and $P_\varphi$ values associated with the VRii regime of Table~\ref{t1}, and for various different values of $c_\beta$. It can, again, be seen that
finite $c_\beta$ has a  larger relative effect on the real part of the response index than on the imaginary part. It can also be seen that the
difference between the predictions of the two models is an increasing function of increasing $c_\beta$, and that the difference is essentially
...
at which the imaginary part of the layer response index, ${\rm Im}(\hat{\mit\Delta})$,  is zero, plotted as a function of $c_\beta$. The calculations are performed
...
RMP on the plasma in the immediate vicinity of the rational surface modifies the ${\bf E}\times {\bf B}$ frequency at the
rational surface in such a manner as to reduce ${\rm Im}(\hat{\mit\Delta})$ to a very small value. In fact, there is a critical RMP amplitude above
which  the normalized ${\bf E}\times {\bf B}$ frequency at the surface, $Q_E$,  ``locks'' to the value that makes ${\rm Im}(\hat{\mit\Delta})$ (almost) zero \cite{rf1998}.
...
torque acts to bring the plasmas into a state in which the electron and ion fluids at the rational surface rotate in opposite directions. 
As $c_\beta$ increases the residual rotation frequency of the electron fluid [i.e., $(Q_E+Q_e)_{\rm crit}$] increases. 
...
\bibitem{elm} T.E.~Evans, R.A.~Moyer, J.G.~Watkins, P.R.~Thomas, T.H.~Osborne, 
