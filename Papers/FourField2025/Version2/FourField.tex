...
to determine the linear two-fluid response of a resonant layer to the perturbation in the outer region. This treatment was extended in Ref.~\cite{rf2022} to take into account the
 ...
 This result is significant because there is some experimental evidence for such a shift \cite{paz}. The first aim of this paper is to demonstrate how the calculation of
...
\caption{The scaled layer response index, $\hat{\mit\Delta}$, plotted as a function of the normalized ${\bf E}\times {\bf B}$ frequency, $Q$, in
...
is then integrated to a small (with respect to unity) value of $p$. The scaled layer response index, $\hat{\mit\Delta}$, is then deduced from  the small-$p$ elements of the solution matrix
...
Theow-$\beta$ approximation used in Refs.~\cite{cole} and \cite{rf2022} involves neglecting the term
...
J.A.~Boedo, M.E.~Fenstermacher, K.H.~Finken, R.J.~Groebner, M.~Groth, et al., {\em Suppression of large edge-localized modes in high-confinement DIII-D plasmas with a stochastic magnetic boundary}, 
