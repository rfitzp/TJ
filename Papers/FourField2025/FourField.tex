...
\author{R.~Fitzpatrick\,\footnote{rfitzp@utexas.edu}\\[2ex]{\em Institute for Fusion Studies,  Department of Physics,}\\[0.5ex]{\em   University of Texas at Austin,  Austin TX, 78712, USA}}
\date{}
\maketitle
%\begin{abstract}
...
to determine the linear two-fluid response of a resonant layer to the perturbation in the outer region. This treatment was extended in Ref.~\cite{rf2022} to take into account the
 ...
 Lee, Park \& Na \cite{lee} recently demonstrated  how to solve the full tenth-order four-field resonant layer equations in configuration  space using a Riccati transformation. In the process, they discovered that, in a high-$\beta$ plasma,  the frequency (in the local ${\bf E}\times {\bf B}$ frame)  at which response of
 a resonant layer to the magnetic perturbation in the outer region is strongest  attains its maximum value is shifted in the ion diamagnetic direction from the electron diamagnetic frequency.
...
The response of the plasma to the peturbation is
governed by linearized, marginally-stable, ideal-MHD everywhere in the plasma, apart from a (radially) narrow
...
across the rational surface, whereas $\partial\delta\psi/\partial r$ is discontinuous. The discontinuity of
 ...
 \begin{equation}
{\mit\Delta\Psi}_{\!s} = \left[r\,\frac{\partial \delta\psi}{\partial r}\right]_{r_{s-}}^{r_{s+}}
\end{equation}
...
direction and $n$ periods in the toroidal direction), leads to the relation \cite{rf1993,fkr}
...
The current sheet at the rational surface can only be resolved by employing  a resistive plasma response model
...
be driven if there were no plasma at the surface. On the other hand, in the ideal response regime, the  current sheet excited in the layer is
...
$\tau_E = r_s^{\,2}/\chi_E$ the energy confinement time.
  Furthermore, $P_\varphi = \tau_R/\tau_\varphi$ and $P_E = \tau_R/\tau_E$ are magnetic Prandtl numbers.
...
\end{align}
Here, $Q=S^{1/3}\,n\,\omega_E\,\tau_H$, $Q_{e,i} = S^{\,1/3}\,n\,\omega_{\ast\,e,i}\,\tau_H$,
...
get from our equations to those of Lee et alia by making the following transformation: $Q\rightarrow -Q_E$, $Q_e\rightarrow Q_{\ast\,e}$, $Q_i\rightarrow Q_{\ast\,i}$, $\iota\rightarrow 1/\tau$, $P_E\rightarrow c_\beta^{\,2}$,
...
\skew{3}\tilde{\phi}(X)&\rightarrow \psi_0\,{\rm i}\,g\,\left[\frac{\skew{6}\hat{\mit\Delta}}{2}\,{\rm sgn}(X) +\frac{1}{X}+ {\cal O}\left(\frac{1}{X^2}\right)\right],\label{e312a}\\[0.5ex]
...
Asymptotic matching to the outer solution  then yields
...
$\bar{N}(p) \rightarrow [-{\rm i}\,Q_e/(g+{\rm i}\,Q_e)]\,\bar{Y}(p) + {\cal O}(p)$ and  $\bar{V}(p)\rightarrow {\cal O}(p^2)$
...
\bar{Y}(p) &= (g+{\rm i}\,Q_e)\,(a_{-1} \,p^{-1}+a_0)+ {\cal O}(p),\\[0.5ex]
...
Note that, because there are no free parameters in expression (\ref{e147}), the layer response parameter is uniquely determined by this procedure. 
